...
- ~~새로운 소비 아이템을 3종 추가한다. (100%)~~
- ~~마나 포션을 사용할 경우 최대 마력이 1 증가한다. (100%)~~
- ~~에센스 포션을 사용할 경우 최대 마력이 2 증가한다. (100%)~~
- ~~흐름의 물약을 사용할 경우 마력이 2 회복된다. (100%)~~
- ~~전투가 끝나면 마력을 최대 마력까지 회복하게 한다. (100%)~~
- 보스 스테이지에서는 배경이 약간 어둡고 붉은색으로 변하게 만든다.
- 모든 보스 스테이지에서 비장한 음악이 흐르게 만든다.
- 보스 스테이지에서는 몬스터 배경이 검은색으로 변하며, 테두리에는 검은 장식이 달리게 변경한다.
- 가방 UI을 끄면 켜져 있는 아이템 정보를 초기화한다.
- 체력 프로세스 바 또는 마력 프로세스 바를 클릭하면 해당 능력치의 설명이 표시되게 변경한다.
- 동결 1단계에 걸릴 경우, 2턴 동안 지속이 되며 그동안 반격 확률이 7% 줄어든다.
- 동결 2단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 반격 확률이 9% 줄어든다.
- 동결 3단계에 걸릴 경우, 4턴 동안 지속이 되며 그동안 반격 확률이 12% 줄어든다.

- 포이즌 그로겔에서 도마뱀 고기 대신 마나 포션이 나오도록 변경한다. ( 확률은 15% )
- 본웜에게서 모어 헬스 포션 대신 마나 포션이 나오도록 변경한다. ( 확률은 15% )
- 프로즌 랫에게서 라이트닝 포션 대신 흐름의 포션이 나오도록 변경한다. ( 확률은 30% )
- 원한의 유령에게서 축복 포션 대신 흐름의 포션이 나오도록 변경한다. ( 확률은 30% )
- 리더 울프에게서 엔드 미트 대신 에센스 포션이 나오도록 변경한다. ( 확률은 15% )
- 사무친 원혼에게서 어스 포션 대신 에센스 포션이 나오도록 변경한다. ( 확률은 15% )

- 쇼크웨이브의 데미지가 100에서 110으로 증가한다.
- 에너지 슬래셔의 데미지가 150에서 165로 증가한다.
- 크레피투스의 데미지가 200에서 220으로 증가한다.
