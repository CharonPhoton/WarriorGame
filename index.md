...
### 12) 15주 조우 요소 추가 패치
**추가 사항**
- 특수 상자를 추가한다. 특수 상자에서는 Wonder 아이템 1~2개, Rare 아이템 3~4개, 총합 4~5개의 아이템이 무조건 들어 있다.
- 특수 상자에서 아이템들 중에서 하나의 아이템을 무작위로 얻을 수 있다. 이 상자의 출현확률은 1/2%다.
- 상점에 튜토리얼을 추가한다.

**변경 사항**

- 내구도 감소량, 점수에 ':' 표시를 추가한다.
- Chapter와 Act에 표시되었던 '.'를 삭제한다.
- 조합법 필터 버튼이 작동 안 하던 버그를 수정한다.
- 조합법의 각 아이템의 박스 우측이 조금 잘리던 현상을 수정한다.
- 사전, 상점 필터 활성화 시에 배경이 추가되도록 변경한다.
- 로비 중앙 버튼, 가방 아이템의 단축키, 아이템 필터 체크박스, 장착 아이템 이벤토리 UI를 변경시킨다.
