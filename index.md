...
### 24) 28주 퀘스트 추가
**추가 사항**
- 3가지 퀘스트를 추가한다. 퀘스트에는 스토리 퀘스트, 반복 퀘스트, 목표 퀘스트가 있다.
- 스토리 퀘스트는 Act 1 여정 모두 클리어, Act 1 클리어 이후 마녀 대화, Act 2 진입 이전 마녀 대화, Act 2 여정 클리어, Act 2 여정 클리어 이후 마녀 대화, 베타 및 데모 안내를 한다.
- 스토리 퀘스트( 스토리 길잡이 ) UI는 홈 화면의 우측에 위치해 있다.
- 반복 퀘스트를 추가한다. 반복 퀘스트는 클리어할 때마다 카운터가 하나 쌓이고, 카운터는 해당 퀘스트 이름의 옆에 가로와 함께 표시된다.
- 반복 퀘스트에는 마물 처치, 보스 처치, 히든 보스 처치, 여정 클리어, 치유의 숲 조우, 이상한 공간 조우, 아이템 조합, 마물에게서 Unique 아이템 / Epic 아이템 / Legend 아이템 얻기 퀘스트가 있다.
- 퀘스트 UI 안에는 퀘스트의 이름, 퀘스트의 내용, 퀘스트의 진행 슬라이더 바와 진행도(숫자), 보상, 보상 받기 버튼이 있다.
- 목표 퀘스트를 추가한다. 목표 퀘스트는 해당 목표를 달성하면 클리어되는 방식이며 반복 퀘스트처럼 카운터가 쌓인다.
- 목표 퀘스트에는 아이템 사전 채우기, 마물 사전 채우기, 보스 기억 아이템 모으기, 아이템이나 특성 해금하기, 아이템이나 특성 강화하기가 있다.
- 더 이상의 목표 퀘스트를 진행할 수 없을 경우, 퀘스트의 맨 위에 체크 표시 아이콘을 띄운다.
- 퀘스트의 출력 순은 보상을 받을 수 있는 퀘스트 > 보상을 못 받는 퀘스트 > 완료된 퀘스트 순이다.

**변경 사항**

- 일부 UI를 개선된 스프라이트로 변경한다.
- 모든 폰트를 변경한다.
