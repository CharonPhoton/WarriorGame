...
### 15) 17주 버그 수정
**추가 사항**
- 접몽 석상의 조합에 이미 넣은 재료를 뺄 수 있는 빼내기 버튼을 추가한다.
- 상점 해금 아이템인 리볼버를 추가한다. 리볼버는 거대 들쥐에게서 나오는 공격력 5 증가, 크리티컬 데미지 10% 증가 효과를 가진 E등급 Normal 총 무기 아이템이다.
- 도감에 미획득 혹은 미처치 요소들만 출력되는 필터를 추가한다.

**변경 사항**

- 스테이지의 챕터와 숫자가 겹치는 현상을 수정한다.
- 중독 상태이상 글자가 누락되는 버그를 수정한다.
- 보상 아이템의 배경 크기 버그를 수정한다.
- 가방 아이템에서 외곽선 달린 칸이 스크롤이 넘어가도 표시되는 버그를 수정한다.
- 가방 아이템의 스크롤 높이가 비정상이던 버그를 수정한다.
- Tip 중에 {Emter}라고 뜨는 팁의 오타를 수정한다.
- 모바일에서 가방의 단축키 옵션이 표시되던 버그를 수정한다.
- 옵션 창이 음량을 조절하면 자동으로 꺼지던 버그를 수정한다.
- 인게임의 옵션 창 음량 조절이 저장되지 않던 버그를 수정한다.
- 특수 상자의 아이템 얻기 소리가 BGM 음량에 따르던 버그를 수정한다.
- 접몽 석상 창의 석상의 크기가 이상하던 버그를 수정한다.
- 랭킹에서 슬로우 ~ 트라이얼 모드의 Act 표기가 이상하던 버그를 수정한다.
- 랭킹에서 슬로우 ~ 트라이얼 모드에서 뉴 스코어 표기가 수시로 뜨던 버그를 수정한다.
- Act 2의 짙은 배경의 효과 크기가 맞지 않던 버그를 수정한다.
- 대화 종료 이후에 이벤트가 있는 대화를 종료하면 이벤트가 발생하지 않던 버그를 수정한다.
- 접몽 석상, 상기 상자, 특수 상자를 열 시에 점수를 주도록 변경된다.
- 구입 소리의 음량을 조금 줄인다.
- 외곽선의 크기를 조정한다.
- 접몽 석상과 조합법에 표기된 재료 아이템들에 누르면 정보가 출력되는 시스템을 추가한다.
- 상기 상자와 특수 상자에 가방 아이템을 추가한다.
- 몽환석을 점수 / 90만큼 얻게 변경한다.
- 육중의 강화 수치가 60에서 45로 하향한다.
