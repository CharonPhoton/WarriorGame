...
- 몬스터의 체력이 0 이하가 될 경우, 플레이어는 전투에서 승리한다.
- 몬스터의 체력이 0 이하가 될 경우, 몬스터가 서서히 사라지는 애니메이션을 출력한 뒤, 전투에서 승리했다는 판정을 내린다.
- 전투에서 승리할 경우, 행동 버튼이 사라진다.
- 전투에서 승리할 경우, 몬스터의 보상 리스트에 있는 보상들 마다 난수를 1~100까지 돌려, 해당 아이템을 얻을 수 있는 확률의 수치보다 나온 난수가 같거나 작을 경우 빈 리스트에 해당 아이템을 넣는다.
- 전투에서 승리할 경우, 플레이어는 리스트에 저장되어 있는 보상을 인벤토리에 추가할 수 있다.
- 아이템을 얻을 수 있는 UI 창은 완료 버튼을 누르면 사라진다.
