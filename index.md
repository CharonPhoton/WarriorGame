...
### 4) 6주 공격 번개 마법 아이템 제작 및 무기 개별화 시작 (A1.012 패치)
**추가 사항**

- 총 타입의 무기와 활 타입의 무기를 추가한다.
- 아이템 데이터베이스에서 무기가 어떤 무기인지 세분화하기 위해서 Type2 테이블을 하나 더 둔다.
- 총 타입의 무기로 공격하면 무언가 발사되는 소리가 출력되며, 크리티컬이 발생될 경우 무언가가 폭발하면서 발포되는 소리가 출력된다.
- 총 타입의 무기로 공격하면 적에게 무언가 피격되는 이펙트가 출력되며, 크리티컬이 발생될 경우 충격파가 일어나며 피격되는 이펙트가 출력된다.
- 활 타입의 무기로 공격하면 무언가가 빠르게 날아가는 소리가 출력되며, 크리티컬이 발생될 경우 무언가가 더 빠르고 강하게 날아가는 소리가 출력된다.
- 활 타입의 무기로 공격하면 무언가가 빠르게 적에게 날아가는 듯한 이펙트가 출력되며, 크리티컬이 발생될 경우 무언가가 회전하며 빠르게 날아가는 듯한 이펙트가 출력된다.
- 감전 1단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 치명타 반격 확률이 본래의 20% 만큼 감소한다.
- 감전 2단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 치명타 반격 확률이 본래의 30% 만큼 감소한다.
- 감전 3단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 치명타 반격 확률이 본래의 40% 만큼 감소한다.
- 몬스터가 감전 상태 이상에 걸렸을 경우, 전기가 일어나고 있는 듯한 연출을 출력한다.
- 번개 속성의 데미지를 주는 마법 아이템은 푸른 타격 이펙트를 출력하게 한다.
- 번개 속성의 데미지를 주는 마법 아이템을 사용하면 저릿저릿한 효과음을 출력하게 한다.
- 몬스터가 감전 상태이상에 걸리면 몬스터가 감전 상태이상에 걸렸다고 밝은 하늘색 텍스트로 출력하게 한다.
- 라이트닝의 데미지는 20이며, 적에게 감전 1단계 상태이상을 부여하며, 등급은 E다.
- 라이트닝 쇼크의 데미지는 35이며, 적에게 감전 1단계 상태이상을 부여하며, 등급은 D다.
- 썬더 볼트의 데미지는 53이며, 적에게 감전 2단계 상태이상을 부여하며, 등급은 C다.
- 썬더 스트라이크의 데미지는 70이며, 적에게 감전 2단계 상태이상을 부여하며, 등급은 A다.
- 썬더 익스플로전의 데미지는 118이며, 적에게 감전 3단계 상태이상을 부여하며, 등급은 R다.
- 풀구르의 데미지는 195이며, 적에게 감전 3단계 상태이상을 부여하며, 등급은 S다.

**변경 사항**

- 블루 울프에게서 매직 애로우 대신 라이트닝이 나오게 변경한다.
- 숲 전갈에게서 매직 스워드 대신 라이트닝 쇼크가 나오게 변경한다.
- 포이즌 웜에게서 슬래쉬 블로우 대신 썬더 볼트가 나오게 변경한다.
- 본웜에게서 쇼크웨이브 대신 플레어 썬더 스트라이크가 나오게 변경한다.
- 스켈레톤에게서 에너지 슬래셔 대신 썬더 익스플로전이 나오게 변경한다.
- 크리스털 스파이더에게서 크레피투스 대신 풀구르가 나오게 변경한다.
