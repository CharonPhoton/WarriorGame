...
**추가 사항**
- 지식 사전을 추가한다.
- 지식 사전의 인물들은 조건에 따라서 열리게 만들며, 잠겨진 인물들은 아이템이나 몬스터처럼 모두 비공개로 만든다.
- 잠겨진 지식은 이미지가 검게 출력되며, 비공개된 텍스트는 모두 ???로 만든다. 
- 지식의 스토리 설명은 분기마다 구분선을 둔다.
- 지식의 내용에는 이름, 대표 이미지, 대표 설명, 스토리 설명이 있다.
- 능력치 소비 아이템을 자동으로 사용하도록 변경한다. 능력치 소비 아이템이 인벤토리에 추가되면 자동적으로 사용되도록 시스템을 만든다.
- 능력치 소비 아이템 자동 사용 시스템은 옵션에서 On/Off로 끄거나 킬 수 있도록 변경한다.

**변경 사항**

- B등급 이상의 고대석, 능력치 소비 아이템을 제외한 모든 아이템들의 최대 강화를 5로 증가한다.
- 보상의 팝업창이 바깥의 검은 부분을 누르면 꺼지도록 수정한다.
