...
- 플레이어의 특성을 플레이어 프리팹으로 저장하게 한다.
- 특성 선택 씬 이전에 로딩 씬을 추가하여, 플레이어의 데이터( 인벤토리, 특성 )를 불러오게 한다.
- 특성 선택 이후 로딩 씬에서는 게임 내 데이터를 불러오게 한다.
- 게임 시작 시, 만약 플레이어의 특성이 있을 경우 곧바로 게임 내 데이터를 불러오는 로딩 화면으로 이동한다.
- 게임 시작 시, 만약 플레이어의 특성이 없을 경우 특성 선택 화면 씬으로 이동한다.
- 플레이어의 체력이 0이 되었을 경우, 게임 오버 UI를 띄운다.
- 플레이어가 게임 오버 UI에서 초기 화면으로 돌아가는 버튼을 누를 경우, 현재까지 있던 플레이어의 인벤토리 데이터와 특성이 초기화된다.
- 플레이어가 진행하기 버튼 오른쪽의 리셋 버튼을 누를 경우, 현재까지 있던 플레이어의 인벤토리 데이터와 특성이 초기화되며, 특성 선택 씬으로 넘어간다.
