...
[만들게 된 배경]  
제한된 인벤토리 슬롯을 가지고 여러 적들을 만나거나 상자를 발견하여 여러 보상을 얻어서 최종 스테이지를 클리어하는 간단한 로그라이크 게임이 있으면 좋겠다고 생각하며 만들었다.
...
- 만약 스킬을 다른 것으로 변경할 경우, 화면 우측에는 해당 캐릭터의 스킬 인벤토리가 뜬다.
...
- ~~홈에서 플레이어가 정한 분대원의 정보를 확인할 수 있다. 정보는 요구사항에 나온 대로 이름, 이미지, 레벨이다. (100%)~~
- ~~분대 관리에서 분대원 모집을 통해서 분대원을 추가할 수 있다. (100%)~~
- ~~모집으로 추가된 캐릭터는 카드로 만들어진다. (100%)~~
- 분대 관리에서 일정 재화를 소비해서 분대원을 강화할 수 있다. (30%)

- ### 11) 11주 - 부대 관리 시스템 제작 : 분대원 정보 편집 및 세부 사항 창 제작

- ~~카드 불러올 때 10번째 이상은 불러오지 못하는 버그 수정 -> for문에 스쿼드 데이터의 양을 참조하던 것이 원인 -> 카드 데이터 양으로 변경 (100%)~~
- ~~카드나 분대원을 누르면 해당 분대원이나 카드의 세부 사항이 나오는 창이 나온다. (100%)~~
- ~~세부 사항 창에서 화면 좌측에는 해당 분대원의 레벨, 경험치, 이름, 이미지, 체력, 공격력, 공격 속도, 에너지, 크리티컬 데미지, 크리티컬 확률이 나온다. (100%)~~
- ~~세부 사항 창의 화면 좌측에는 다음이나 이전 칸에 있는 카드나 분대원의 정보를 볼 수 있는 버튼이 있다. (100%)~~
- ~~이전 또는 다음 카드나 분대원의 정보를 확인하는 버튼을 누르면 분대원은 다음이나 이전 분대원의 정보를, 카드는 다음이나 이전 카드의 정보로 넘어간다. (100%)~~
- ~~세부 사항 창의 화면 좌측에는 스텟을 올릴 수 있는 버튼이 존재한다. (100%)~~
- ~~세부 사항 창에서 화면 중앙에는 나가기, 아이템, 장비, 스킬을 장착할 수 있는 버튼이 있다. (100%)~~
- ~~장착하고 있는 아이템, 장비, 스킬을 편집하는 버튼에는 현재 장착하고 있는 아이템, 장비, 스킬의 이미지가 뜨며, 그 아래에는 그 이름이 뜬다. (100%)~~
- ~~화면 우측에는 무언가를 편집하게 되면 그에 맞춰서 UI가 변경된다. (100%)~~
- ~~만약 아이템을 다른 것으로 변경할 경우, 화면 우측에는 아이템 인벤토리가 뜬다. (100%)~~
- ~~만약 장비를 다른 것으로 변경할 경우, 화면 우측에는 장비 인벤토리가 뜬다. (100%)~~
- ~~만약 스킬을 다른 것으로 변경할 경우, 화면 우측에는 해당 캐릭터의 스킬 인벤토리가 뜬다. (100%)~~
- ~~만약 아무 것도 장착하지 않은 것이 있다면 그 칸은 빈칸으로 표시된다. (100%)~~
- ~~인벤토리 칸은 한 줄에 최대 4칸이 뜬다. (100%)~~
- ~~인벤토리에 있는 아이템이나 스킬을 누르면 현재 장착하고 있는 아이템 및 스킬과 바뀐다. (100%)~~
- ~~장착하고 있는 아이템을 인벤토리가 켜진 상태로 누르면, 장착 아이템이 해제된다. (100%)~~
- ~~저장 방식을 바꾼다. ( 이전 : 베이스 데이터 파일, 세이브 데이터 파일을 불러와서 세이브 데이터 파일을 베이스 데이터 파일에 덮어씌운다. -> 변경 : 세이브 데이터 파일만을 불러서 적용한다. )[ 이것은 이후에 하게 될 수도 있다. ] (100%)~~

- ### 12) 12주 - 인벤토리 제작 : 인벤토리 씬, 불러올 데이터 제작 시작, 스텟 올리기 제작

- ~~인벤토리 씬을 제작한다. (100%)~~
- ~~인벤토리 씬을 불러올 때 초기에는 소비 아이템 인벤토리 창이 뜬다. (100%)~~
- ~~화면 좌측에는 위 순서로 소비 아이템, 장착 아이템, 재료 아이템 버튼이 있다. (100%)~~
- ~~소비 아이템을 누르면 보유 중인 소비 아이템이 그리드 레이아웃 형식으로 뜬다. (100%)~~
- ~~장착 아이템을 누르면 보유 중인 소비 아이템이 그리드 레이아웃 형식으로 뜬다. (100%)~~
- ~~재료 아이템을 누르면 보유 중인 소비 아이템이 그리드 레이아웃 형식으로 뜬다. (100%)~~
- ~~해당 아이템을 누르면 화면 우측에서 해당 아이템의 이미지, 이름, 내구도 혹은 보유 갯수, 설명, 효과가 나타난다. (100%)~~
- ~~아이템의 정보는 모두 csv 파일로 불러온다. 이 때, csv 데이터에는 아이템의 이름, 내구도, 설명, 효과가 각각 들어있다. (100%)~~
- ~~플레이어의 재화 ( 코인 )은 플레이어 프리팹으로 불러온다. (100%)~~
- ~~플레이어의 레벨은 플레이어 프리팹으로 불러온다. (100%)~~
- ~~플레이어의 경험치는 플레이어 프리팹으로 불러온다. (100%)~~
- ~~분대원이나 카드를 드래그 할 때, 해당 캔버스를 벗어나면 사라지는 버그 수정 -> 최상위 캔버스에 빈 오브젝트를 만들어서 드래그 할 때에는 그 오브젝트를 부모로 삼고, 아닐 경우 해당 캔버스를 다시 부모로 삼는 방식으로 해결 (100%)~~
- ~~(수정 사항)분대원 정보 창에서 나가기 버튼을 화면 좌측에 위치하도록 변경 (100%)~~
- ~~분대원이나 카드의 정보에서 스텟 올리기 버튼을 누르면 화면 중앙과 우측이 모두 스테이터스 증가에 관련된 패널로 바뀐다. (100%)~~
- ~~스텟 올리기 버튼을 누를 경우, 화면 중앙에는 체력, 공격력, 공격 속도, 에너지, 크리티컬 데미지, 크리티컬 확률을 올릴 수 있는 버튼이 나타난다. (100%)~~
- ~~스텟을 올리는 창에는 세로로 6개의 그룹이 있으며, 각 그룹마다 해당 스텟 증가량, 현재 스텟 투자량, 투자 버튼이 나온다. (100%)~~
- ~~스텟은 현재 레벨만큼 증가할 수 있다. (100%)~~
- ~~스텟은 각각 체력 + 10, 공격력 + 1, 공격 속도 -0.02, 에너지 + 2, 크리티컬 데미지 + 1%, 크리티컬 확률 + 0.1%다. (100%)~~
