...
### 7) 7주 본격 스토리 추가
**추가 사항**
- 해당 상황에서의 대화를 추가한다. ( 상점에 있는 마녀는 가능한 대화가 있다면 마녀를 누르면 대화 목록을 뜨게 만든다. )
- 다음 상황에서의 대화를 추가한다. ( 엔딩 이후 홈 화면, 엔딩 이후 마녀 대화, 어둠 속에 존재하는 자 대면 이후 마녀 대화, 너클 종류 무기 획득 후 마녀 대화, 강몽석과 재몽석 마녀 대화, 여정과 상점 마녀 대화, 이상한 공간 마녀 대화, 치유의 숲 마녀 대화 )
- 복구석을 추가한다. 복구석은 최근에 파괴된 특정 종류의 아이템을 부활시킬 수가 있다. 복구석은 챕터 4~6의 몬스터에게서 나오는 S등급 에픽 아이템이다.
- 긍지의 복구석은 최근에 파괴된 무기 아이템을 내구도를 50%로 하여 복구시킨다. 긍지의 복구석은 챕터 4~6에서 나오는 첫 번째 몬스터에게서 3% 확률로 나온다.
- 의지의 복구석은 최근에 파괴된 방어구 아이템을 내구도를 50%로 하여 복구시킨다. 의지의 복구석은 챕터 4~6에서 나오는 두 번째 몬스터에게서 3% 확률로 나온다.
- 통찰의 복구석은 최근에 파괴된 장신구 아이템을 내구도를 50%로 하여 복구시킨다. 통찰의 복구석은 챕터 4~6에서 나오는 세 번째 몬스터에게서 3% 확률로 나온다.
- 한계의 복구석은 최근에 파괴된 부적 아이템을 내구도를 50%로 하여 복구시킨다. 한계의 복구석은 챕터 4~6에서 나오는 네 번째 몬스터에게서 3% 확률로 나온다.
- 새로운 아이템인 ?? ?????를 추가한다. ?? ?????은 R등급 Legend 너클 종류의 무기 아이템으로 공격력 60, 방어력 17, 반격 보정 확률 7%, 크리티컬 데미지 100%, 크리티컬 확률 7%를 제공한다. 스포아, 바드, 켈트에게서 1% 확률로 나온다.
- 새로운 특성인 육중을 추가한다. 육중은 체력을 120 제공하는 특성으로, 강화하면 180 체력을 제공한다.
- 고대석을 제외한 A등급 아이템의 강화를 4단계까지 가능하게 한다. 

**변경 사항**

- 끈기 + 1의 체력 증가량이 50에서 40으로 감소한다.
- 집중의 물약의 가격을 8에서 12으로 증가한다.
- 보스 경고 튜토리얼이 2 챕터 이후부터 나오도록 수정한다.
- 사전의 대화집에서 플레이어라는 이름을 용사로 수정한다.
- 대화가 겹치면 차례로 출력되도록 수정한다.
- 사전이 제대로 작동되지 않던 버그를 수정한다.
