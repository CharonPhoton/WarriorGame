...
- 대부분 UI를 모두 개선된 디자인으로 준비된 UI 스프라이트들로 교체한다.
- 단축키로 생성된 가방 창은 마우스 아래가 아닌 모바일과 동일한 위치에서 생성되도록 수정한다.
- Alt + Enter를 방지하도록 유니티에서 설정한다.
- Act 1 엔딩 이후 한글 마녀 대사의 오타를 수정한다.
- 영어 아이템 데이터가 잘 불러오지 못했던 버그를 수정한다.
- 튜토리얼, 대화 넘기기, 보상 완료에 스페이스 바 단축키를 추가한다.
- 내구도 튜토리얼의 박스 위치가 이상하던 버그를 수정한다.
