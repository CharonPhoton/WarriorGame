...
### 18) 20주 가방 개선 작업
**추가 사항**
- 가방을 얻을 경우, 옵션 기능이 켜져 있으면 해당 가방 아이템에 대한 선택지 팝업창이 나오도록 변경한다.
- 가방 선택지 팝업창에서 얻을 가방 아이템보다 수용량이 적은 가방 아이템을 누르면 내용물 옮기기, 가방 바꾸기 버튼이 활성화된다.
- 가방 선택지 팝업창에서 내용물만 옮기기 버튼을 누르면 해당 선택 가방 아이템의 내용물들이 얻을 가방 아이템을 얻고, 그 안에 모두 이동된다.
- 가방 선택지 팝업창에서 가방 바꾸기 버튼을 누르면, 선택한 가방 아이템을 내용물만 추출하고 삭제한 뒤, 얻을 아이템을 얻고 그 안에 모두 이전 가방의 내용물들을 채워넣는다.
- 가방 선택지 팝업창에서 가방이 8개가 있으면 그냥 얻기 버튼을 받지 않기 버튼으로 바꾼다.
- 가방 선택지 팝업창에서 가방이 7개 이하 있으면 받지 않기 버튼을 그냥 얻기 버튼으로 바꾼다.
- 새로운 특성인 증력을 추가한다. 증력 특성은 최대 마력을 3 올려주는 기본 특성이다.
- 메인 홈, 게임 설정, 특성, 상점 튜토리얼을 모두 보면 홈 화면에서 보상을 주도록 만든다. ( 각각 보상은 몽환석 5,000, 강몽석 50, 몽환석 5,000, 재몽석 1이다. )

**변경 사항**

- 홈 화면의 버튼들에 마우스를 올리거나 누르면 하얀 하이라이트가 출력되도록 변경한다.
- 인게임 옵션에 검은 뒷배경이 출력되지 않던 버그를 수정한다.
- 아이템 구입창의 아이템 종류 텍스트 출력 버그를 수정한다.
- 탐색 특성의 유물이라는 단어를 상자로 롤백한다.
- 로그의 길이를 화면의 높이와 똑같게 만든다.
- 일부 아이템 설명을 변경하며 오타를 수정한다.
- 탐색, 제약, 마강 특성을 상점에서 해금해야 할 수 있는 특성으로 변경한다.
- 특성 가격을 3,000으로 변경한다.
- 마강의 최대 마나 버그를 수정한다.
- 엔딩의 몽환석 지급량이 낮게 표시되던 버그를 수정한다.
- UI 위치를 조정한다.
