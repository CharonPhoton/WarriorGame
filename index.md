...
- 보물 상자를 조우할 경우, 전투 화면에서 보물 상자가 나타난다.
- 보물 상자를 조우할 경우, 행동하기 버튼이 열기 버튼으로 바뀐다.
- 보물 상자를 열 경우, 스테이지 데이터에 저장 되어 있는 현재 쳅터에 맞는 보물 상자 보상이 나온다.
- 보물 상자를 열어서 나온 보상들은 각각 난수를 돌려, 그 수가 해당 아이템의 드랍률보다 같거나 낮을 경우에만 나온다.
- 나온 아이템들은 보상 UI에서 나오며, 플레이어는 보상 창에서 아이템을 얻을 지, 말지를 결정할 수 있다.
- 완료하기 버튼을 누르면, 보상 UI가 꺼진다. 그 후, 다음 스테이지로 넘어간다.
- 스테이지가 넘어가면, 난수를 돌린다. 나온 난수가 몬스터 조우 확률보다 같거나 낮으면 몬스터와 조우한다. 아닐 경우에는 선물 상자와 조우한다.
- 몬스터와 조우할 경우, 난수를 돌린다. 각 몬스터 조우 확률을 스테이지 데이터에서 확률이 낮은 순서대로 비교한다.
- 만약에 난수가 해당 몬스터와의 조우 확률 보다 같거나 낮을 경우 해당 몬스터와 전투한다. 만약 아닐 경우, 변수 하나에 해당 몬스터의 조우 확률을 더한다.
- 만약 다음 몬스터의 조우 확률을 비교할 경우, 그 몬스터의 조우 확률에 저장한 변수의 값을 더한 수치로 비교한다.
