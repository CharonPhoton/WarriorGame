...
### 37) 47주 사념 제련소 작업
**추가 사항**
- 사념 제련소를 추가한다. 사념 제련소는 1% 확률로 나오는 것으로, 5번의 기회를 주어서 아이템의 내구도를 수리하거나 아이템을 강화할 수 있다.
- 사념 제련소의 슬롯에 아이템을 드래그 앤 드랍을 할 수 있도록 만든다.
- 사념 제련소의 슬롯에 있는 아이템이 내구도가 없거나 내구도가 모두 찬 상태일 경우, 수리가 불가능하게 버튼을 비활성화시킨다.
- 사념 제련소의 슬롯에 있는 아이템이 R등급 이하의 Normal이나 Rare 아이템이 아닐 경우, 강화하기 버튼을 비활성화시킨다.
- 사념 제련소의 슬롯에 있는 아이템을 강화하면 2%로 다음 랭크 아이템의 다음 희귀성 아이템이 나오며, 3%로 다음 랭크 아이템의 같은 희귀성 아이템이 나오며, 20%로 다음 희귀성 아이템이 나오며, 25% 확률로 같은 등급이며 같은 희귀성 아이템이 나오며, 40% 확률로 강화에 실패하며, 10%확률로 아이템이 파괴된다.

**변경 사항**

- 가방 아이템 종류가 보상의 맨앞에 나오도록 변경한다.
- 1~2 챕터에서는 접몽 석상 대신에 사념 제련소가 나오도록 변경한다.( 이때, 기회는 2번으로 만든다 )
