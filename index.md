...
- 만약 리스트에 저장된 보상이 없을 경우, 아무 보상도 받지 않았다는 문구를 대신 띄운다.
- 보상 UI에서 해당 아이템을 누르면 그 아이템의 설명이 나온다. 단, 장착과 삭제 버튼은 사라지거나 나타나지 않는다.
- 아이템을 얻을 수 있는 UI 창은 완료 버튼을 누르면 사라진다.
- 보상 UI를 끄기 전까지는 행동 버튼이 나오지 않는다.
