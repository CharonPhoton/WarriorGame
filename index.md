...
- ~~몬스터에게 피격을 받으면 애니메이션 이후에 데미지가 달게 변경된다. (100%)~~
...
- ~~그로겔에게서는 신선한 고기(60%), 기력의 간(35%), 힘의 물약(40%), 기력버섯(30%), 최상급 고기(30%), 마골25%), 그로겔 이어링(3%)가 나온다. (100%)~~
...
- ~~버섯인간 스포아게서는 신선한 고기(60%), 기력의 간(35%), 속도의 물약(40%), 천선어(50%), 금괴(40%), 기력의 간(75%), 라이트 해머(60%), 스포아의 이어링(50%)가 나온다. (100%)~~
  ...
  - 모든 장비에 내구도를 추가한다.
  - 모든 아이템에 등급을 추가한다. 등급은 모두 E,D,C,B,A,AH,S,H,X,L,M,G,Z,I가 있다.
  - 반지의 이름을 깨진 반지로 변경한다.
  - E등급 아이템에는 사과, 주괴, 목걸이, 가죽 갑옷, 코인, 깨진 반지, 젬이 해당된다.
  - D등급 아이템에는 벨트, 소형 포션, 검이 해당된다.
  - C등급 아이템에는 고기, 철 갑옷, 도끼, 트레저 부츠, 청옥석이 해당된다.
  - B등급 아이템에는 가방, 대검, 은 주괴, 신선한 고기, 열쇠, 아이언 링이 해당된다.
  - A등급 아이템에는 실프 헬멧, 은 갑옷, 트레저 팬츠, 푸른 마법책, 레이피어, 피의 목걸이, 원한의 해골, 붉은 보석, 보은의 뼈, 힘의 물약, 속도의 물약, 집중의 물약, 괴력육이 해당된다.
  - AH등급 아이템에는 거미의 독니, 기력의 간, 배틀 엑스, 강철 갑옷, 라피스 라줄리 목걸이, 철갑 갑피, 월도, 전투 방패, 활이 해당된다.
  - S등급 아이템에는 들쥐 부적, 블루 울프의 갈기, 포에나, 최상급 고기, 누군가의 쪽지, 바바리안 스워드, 마골, 엘프 넥클레스, 드워프 벨트, 고대서, 아메시스 헬멧, 블루 트레저 팬츠, 사우전드 우드 브레이서, 금괴가 해당된다.
  - H등급 아이템에는 코팅 부츠, 기력버섯가 해당된다.
  - X등급 아이템에는 스크롤, 슬라임의 보은, 전갈 반지, 그린 울프 갈기 망토, 포이즌 글라디우스, 스팀팩, 블러드 아이언, 하프 엘렉서가 해당된다.
  - L등급 아이템에는 라이트 해머, 그로겔 이어링, 독 빼낸 고기, 둠 아머, 천선어가 해당된다.
  - M등급 아이템에는 그림 슬라임 포션, 흉포한 도끼, 레드 에너지 팬츠, 원혼이 담긴 반지, 포이즌 블레이드, 예리한 울프 스워드, 골검, 라바 부츠, 낡은 코인, 스포아의 이어링이 해당된다.
  - G등급 아이템에는 정제된 블루 스콜피온의 독, 프로즌 랫 에너지 아머, 라스 웜의 가죽 장갑, 불의 격통, 검은 반지, 스켈레톤 블레이드, 원한 서린 코인이 해당된다.
  - Z등급 아이템에는 크리스털 스파이더의 수정, 리더 울프의 고기, 남겨진 왕관, 매드 본웜의 갈비뼈, 어둠을 머금은 칼, 크리스털 스콜피온 쌍검, 마스터 케로베로스의 발톱, 부정한 악마의 마법, 바드의 갑옷이 해당된다.
  - I등급 아이템에는 투명한 엘릭서, 켈트의 낡은 검이 해당된다.
  - E등급의 내구도는 5다.
  - D등급의 내구도는 6이다.
  - C등급의 내구도는 8이다.
  - B등급의 내구도는 10이다.
  ...
  - Z등급의 내구도는 28이다.
  - I등급의 내구도는 30이다.
  - 장비 아이템의 내구도는 전투가 끝나면 1씩 줄어든다.
  - 전투가 끝난 후, 장착 중인 장비 아이템의 내구도가 0이 될 경우, 해당 아이템은 파괴된다.
  - 내구도는 각 아이템 아래에 표기한다.
  - 가방 아이템의 아래에는 가방 아이템의 이름이 표기되지 않고, 해당 가방 아이템의 현재 사용 중인 슬롯과 수용한 가능한 아이템의 최대 갯수가 표기된다.
  - 아이템의 속성 칸에 해당 아이템의 등급을 위에 표시하고, 아래에는 해당 아이템의 종류를 표시한다.
  - 아이템의 속성 칸에 마우스를 올리면 속성 칸 위에 등급 순서를 표시한다.
  - 아이템의 속성 칸에 마우스를 떼면 표시되었던 등급 순서를 안 보이게 만든다.
