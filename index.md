...
### 11) 15주 쳅터1 제작
- 쳅터1에 나올 아이템과 몬스터를 추가한다.
- 각 몬스터에게 처치 시 1% 확률로 얻을 수 있는 강력한 아이템을 하나씩 추가한다.
- 사과의 회복 능력을 5로 하향한다.
- csv에 ,가 있으면 불러올 때 split로 자를 경우 생기는 버그가 수정되었다. -> csv 파일에는 , 대신 &를 넣었고, 데이터를 불러올 때 split 이후, Replace를 통해 &를 ,로 바꾸었다.
- 플레이어가 공격에 성공할 경우, 칼로 베는 소리를 출력한다.
- 플레이어가 공격에 실패하여 반격 당할 경우, 주먹에 맞는 소리를 출력한다.
- 플레이어가 아이템을 얻을 경우, 물건을 뒤적이는 소리를 출력한다.
- 플레이어가 사망할 경우, 비명을 지르는 소리를 출력한다.
...
이제 포기하기 버튼을 누르거나 체력이 0이 되면, 인벤토리의 아이템들이 모두 리셋된다.
드디어 모든 준비를 맞추었다.
이제 다음 주부터 쳅터 1에 나오는 적과 아이템들을 더 추가할 예정이다.
