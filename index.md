...
|121|마법 모자|가방|AH|<img src="./Items/Magic Hat.png" width="64">|여러 가지 물건을 담을 수 있는 도구로 사용되기도 하는 모자다.|어차피 내 머리에는 안 맞으니까, 그냥 주머니로 사용하자…|12개의 아이템을 안에 넣을 수 있다.|
...
- 한 챕터 당 라운드를 25에서 20으로 바꾼다. 이때, 마왕은 120라운드에서 등장하도록 수정한다.
- 음량을 조절할 수 있는 시스템을 만든다. 음량은 스크롤 바로 조정할 수 있도록 수정한다.
- 음량을 수정하는 스크롤 바는 홈 화면에서 How To Play 버튼 아래에서 오디오 버튼을 추가해서, 그 버튼을 누르면 나오는 팝업창에 띄우게 한다.
- 또한, 음량을 수정하는 스크롤 바는 게임 도중에 옵션에 오디오 버튼을 포기하기와 종료하기 버튼 중간에 둬서, 그 버튼을 누르면 나오게 만든다.
- 조절한 음량은 float값으로 PlayerPrefs으로 저장하며, 다른 씬으로 넘어가거나 게임을 시작할 때 값을 불러와서 이전에 플레이어가 저장한 음량을 불러오게 한다.
- 로딩 중에는 아이템의 종류와 그 종류의 아이템이 어떤 효과를 지녔는지에 대한 설명과 스테이터스에 대한 설명이 나오도록 수정한다.
- 로딩 중에 시간이 얼마나 남았는지 슬라이더로 표시한다.
- 가방 아이템들의 공간을 모두 하향한다. ( 벨트 : 2개, 가방 : 4개, 마법 모자 : 6개, 스크롤 8개 )
