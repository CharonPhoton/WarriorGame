...
### 3) 4~5주 공격 화염 마법 아이템 제작 및 상태 이상 추가 (A1.010 패치)
...
- 가방 팝업창이 누르면 마우스 아래에 출력되게 변경한다.
- ~~상태 이상 이미지의 위에 몇 턴 남았는지 표시한다. (100%)~~
- ~~화상 1단계에 걸릴 경우, 2턴 동안 지속이 되며 그동안 받는 피해가 10%만큼 증가합니다. (100%)~~
- 화상 2단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 받는 피해가 15%만큼 증가합니다.
- 화상 3단계에 걸릴 경우, 4턴 동안 지속이 되며 그동안 받는 피해가 20%만큼 증가합니다.
- 같은 종류의 상태 이상이 이미 높은 단계로 걸려 있을 경우에는 그 상태 이상은 적용되지 않는다. ( 화상 2단계가 적용된 상태면, 화상 1단계는 적용되지 않는다. )
- 같은 종류의 상태 이상이 이미 낮은 단계로 걸려 있을 경우에는 적용되어 있는 상태 이상 대신에 지금 추가될 상태 이상이 적용된다. ( 화상 1단계가 적용된 상태면, 화상 1단계 상태 이상이 사라지고 화상 2단계 효과가 상대에게 부여된다. )
...
- ~~몬스터가 화상 상태 이상에 걸리면 몬스터가 화상 상태 이상이 걸렸다고 주황색 텍스트로 출력하게 한다. (100%)~~
- 새로운 마법 아이템을 6개 추가한다.
- ~~스파크의 데미지는 15이며, 화상 1단계 상태 이상에 걸리며, 등급은 E다. (100%)~~
- ~~거대 들쥐에게서 매직 애로우 대신 스파크가 나오게 변경한다. (100%)~~
- ~~파이어 슛의 데미지는 30이며, 화상 1단계 상태 이상에 걸리며, 등급은 D다. (100%)~~
- ~~블랙 슬라임에게서 매직 스워드 대신 파이어 슛가 나오게 변경한다. (100%)~~
- ~~파이어 불렛의 데미지는 45이며, 화상 2단계 상태 이상에 걸리며, 등급은 B다. (100%)~~
- ~~흉포한 쥐에게서 슬래쉬 블로우 대신 파이어 불렛가 나오게 변경한다. (100%)~~
- ~~플레어 쇼크웨이브의 데미지는 80이며, 화상 2단계 상태 이상에 걸리며, 등급은 A다. (100%)~~
- ~~라바 슬라임에게서 쇼크웨이브 대신 플레어 쇼크웨이브가 나오게 변경한다. (100%)~~
- ~~레드 블래스터의 데미지는 125이며, 화상 3단계 상태 이상에 걸리며, 등급은 R이다. (100%)~~
- ~~파이어 케로베로스에게서 에너지 슬래셔 대신 레드 블래스터가 나오게 변경한다. (100%)~~
- ~~인페르노의 데미지는 170이며, 화상 3단계 상태 이상에 걸리며, 등급은 S다. (100%)~~
- ~~부정한 악마에게서 크레피투스 대신 인페르노가 나오게 변경한다. (100%)~~
- 가방의 제공 인벤토리 칸이 6칸으로, 마법 모자의 제공 인벤토리 칸이 8칸으로, 스크롤의 제공 인벤토리 칸이 10으로 늘어난다.
- 가방의 등급을 B로, 마법 모자의 등급을 R로, 스크롤의 등급을 S로 변경한다.
- 주머니 아이템을 추가한다. 주머니 아이템은 가방 아이템으로, 인벤토리 칸을 4개까지 제공한다.
- 주머니 아이템의 등급은 D이다.
- 1챕터의 상자에서 주머니가 15%의 확률로 나오게 변경한다.
- 2챕터에서 가방 대신에 주머니가 등장하게 한다.
- 티스 웜이 실프 헬멧 대신 붉은 보석을 드랍하게 변경 ( 확률은 20% )
- 포이즌 웜이 붉은 보석 대신 주머니를 드랍하게 변경 ( 확률은 40% )
- 3챕터의 상자에서 가방이 20%로 나오게 변경
- 4챕터의 몬스터에서 마법 모자 대신 가방이 나오게 변경
- 4챕터의 상자에서 마법 모자가 10%의 확률로 나오게 변경
- 5챕터의 상자에서 스크롤이 10%의 확률로 나오게 변경
- 5챕터의 몬스터들에게서 드랍되는 마법 모자의 확률을 33%에서 30%으로 변경
- 5챕터의 몬스터들에게서 드랍되는 스크롤의 확률을 25%에서 20%로 변경
