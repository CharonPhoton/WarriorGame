...
- ~~본웜은 60% 확률로 나오며, 체력은 125, 데미지는 25, 반격 확률은 55%, 치명타 반격은 15%다. (100%)~~
...
- ~~유령은 50% 확률로 나오며, 체력은 110, 데미지는 45, 반격 확률은 45%, 치명타 반격은 25%다. (100%)~~
- ~~유령에게서는 신선한 고기(55%), 버닝 포션(20%), 천선어(30%), 세월의 갑옷(25%), 홀리 헬멧(20%), 낡은 코인(3%)이 나온다. (100%)~~
- ~~고블린 바드는 50% 확률로 나오며, 체력은 750, 데미지는 45, 반격 확률은 60%, 치명타 반격은 30%다. (100%)~~
...
- 테스트로 받은 피드백 및 보안할 부분을 모두 수정한다.
- 2 쳅터에서 기력의 간 안 나오게끔 수정
- 철 갑옷의 방어력 제공량을 6으로 하향
- 2 쳅터의 몬스터 공격력 10% 증가
- 3 쳅터의 몬스터 공격력 15% 증가
- 아이템을 선택한 채로 가방에 넣으면 아이템 정보가 갱신 안 되던 버그 수정
- 속도의 물약 드랍율 1/2만큼 하향
- 스포아의 이어링, 블러드 아이언 드랍율 1/2만큼 하향
- 3 쳅터의 기력버섯 드랍율 하향, 천선어 드랍 X
- 3 쳅터의 금괴와 마골의 드랍율 1/4만큼 하향
- 4 쳅터의 버닝 포션 드랍율 1/4만큼 하향
- 스포아의 이어링 효율 하향
- 쳅터 마지막 스테이지에서는 무조건 보스가 출현하도록 수정
- 블루 울프 체력 5 증가
- 3 쳅터부터 내구도가 2 닳게 수정
