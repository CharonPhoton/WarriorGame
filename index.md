...
- 상태 이상 이미지나 특성 이미지를 클릭하면 그 상태 이상의 설명 혹은 특성의 설명이 그 아래에 출력되게 한다.( 팝업창을 사용한다. )
- 상태 이상 이미지를 클릭하거나 특성 이미지를 클릭하며 설명 팝업창이 켜진 상태일 경우, 다른 곳을 누르면 팝업창이 꺼지도록 만든다.
...
- 새로운 마법 아이템을 6개 추가한다.
- ~~스파크의 데미지는 15이며, 화상1 상태 이상에 걸리며, 등급은 E이다. (100%)~~
- 거대 들쥐에게서 매직 애로우 대신 스파크가 나오게 변경한다.
