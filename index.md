...
- 동결 1단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 반격 확률이 본래의 8% 만큼 감소한다.
- 동결 2단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 반격 확률이 본래의 10% 만큼 감소한다.
- 동결 3단계에 걸릴 경우, 3턴 동안 지속이 되며 그동안 몬스터의 원래 반격 확률이 본래의 12% 만큼 감소한다.
- 몬스터가 동결 상태 이상에 걸렸을 경우, 얼어붙고 있는 연출을 출력한다.
- 빙결 속성의 데미지를 주는 마법 아이템은 푸른 타격 이펙트를 출력하게 한다.
- 빙결 속성의 데미지를 주는 마법 아이템을 사용하면 얼어붙는 듯한 효과음을 출력하게 한다.
- 몬스터가 동결 상태이상에 걸리면 몬스터가 동결 상태이상에 걸렸다고 푸른 텍스트로 출력하게 한다.
- 새로운 소비 아이템 3개, 마법 아이템을 6개 추가한다.
- 마나 포션을 사용할 경우, 최대 마력이 1 증가한다.
- 흐름의 포션을 사용할 경우, 마력이 2 회복된다.
- 에센스 포션을 사용할 경우, 최대 마력이 2 증가한다.
**변경 사항**
