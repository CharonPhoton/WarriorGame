...
- 새로운 특성을 세 개 추가한다. 이 특성은 모두 상점에서 구입이 가능하도록 변경한다.
- 선취 특성은 전투가 시작 되면 플레이어에게 선취 잔류 효과를 부여한다.
- 선취 잔류 효과 동안 플레이어나 적의 공격력과 방어력이 25%만큼 증가한다.
- 금강 특성은 적에게 치명타를 받을 경우, 그 피해를 85%로 감소시키는 효과가 있다.
- 진귀 특성은 Unique 희귀성 아이템의 출현 확률을 2% 올려주는 효과가 있다.
- 상점의 특성 해금 버튼을 누르면, 전시장에 특성 상품들을 출력시킨다.
- 특성 상품을 누르면, NPC의 대화창에 그 특성에 대한 설명을 출력시킨다. ( 특성의 이미지는 아이템처럼 NPC 중앙에 출력되게 한다. )
- 특성 해금에는 필터가 존재하지 않으니, 필터 버튼을 활성화시키지 않는다.
- 특성을 정말 구입할 거냐고 재차 물어보는 팝업창에는 이름과 챕터가 나오는 아이템과 달리 이름만 출력되도록 한다.
- 특성 구입 시 나오는 NPC의 대사는 아이템 구입 시 나오는 NPC의 대사와 똑같게 만든다.

**변경 사항**

- 게임 데이터의 보안을 강화한다.( 자세한 설명은 기밀이기에 넣지 않는다.(?) )
- 특성을 선택하는 씬에서 특성이 한 가로 줄에 네 개가 나오도록 변경한다.
- 특성의 이름이 특성의 이미지 오른쪽에 표기되도록 수정하며, 특성의 이미지를 살짝 왼쪽으로 이동시킨다.
- 특성을 선택하는 씬에서 특성의 효과와 스토리 설명 사이에 구분선을 추가한다.
- 인게임의 아이템 정보에 구분선을 넣어서 설명들의 구분을 정확하게 해 놓는다. ( 예 : 효과 설명 ( 구분선 ) 설명 )
- 로딩 중 아이템의 이름 색을 희귀성에 따라서 변경되도록 수정한다.
