...
- 새로운 챕터로 넘어갈 때 내구도 감소량이 증가하면, 화면에 감소량이 증가한다고 출력한다.

**변경 사항**

- ~~스크롤 바를 통해서 특성들을 정렬시킨다. 그리고 스크롤 바를 내려서 다른 특성들도 볼 수 있게 변경한다. (100%)~~
- 보상을 받을 때, 인벤토리가 꽉 찼다는 안내를 보상 화면 중앙에 출력하게 만들며, 보상 화면을 어둡게 만든다.
- 진행하기, 공격하기, 완료, 옵션 버튼의 텍스트 왼쪽에 그에 맞는 아이콘을 둔다. ( 각각 오른쪽 포인터, 검, 체크, 두 개의 맞물려진 톱니바퀴 )
- 보상이 없을 때 나오는 능력치 강화 화면의 능력치 표시 이미지를 버튼에서 뒷배경이 있는 프레임으로 변경한다.
