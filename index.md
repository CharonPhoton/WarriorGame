...
### 14) 16주 새로운 모드 추가
**추가 사항**
- 새로운 모드인 Slow, Very Slow, Trial을 추가한다. 이 모드들의 특징은 각각 한 챕터의 스테이지를 25/30/40개로 늘린다.
- Noraml, Fast, Very Fast와 별개로 또 Slow, Very Slow, Trial 랭킹을 추가한다.
- 아이템 정보 확대하기 버튼을 추가한다. 해당 버튼은 아이템 잠금 좌측에 위치한다.
- 아이템 정보 확대하기 버튼을 누르면, 해당 아이템의 정보가 화면의 중앙에 더 큰 화면으로 출력된다. 단, 버튼들은 추가되지 않는다.
- 새로운 조합 아이템 샤이닝 그레이트 소드를 추가한다. 샤이닝 그레이트 소드는 SR등급 Wonder 검 무기 아이템으로, 공격력을 104 증가해준다. ( 조합법은 대검 + 은 검 + 샤이트 라이플 + 라이트 라이플 )
- 스테이지 현황 UI를 클릭하면 전체 스테이지 혹은 해당 챕터의 스테이지 진행 현황이 뜨도록 시스템을 추가한다.

**변경 사항**

- 일부 UI가 깨지지 않도록 크기와 위치를 조정한다.
- 옵션 팝업창에서 다른 쪽을 클릭하면 꺼지도록 변경한다.
- 거목의 뿌리의 강화에 필요한 강화석 비용이 2 더 적었던 버그를 수정한다.
- 조합 아이템 정보 중에 만약에 얻지 않은 아이템일 경우, 해당 아이템을 재료로 하는 아이템이 없다는 문구가 출력되지 않던 버그 수정한다.
- 일부 대화 중 너무 긴 것은 대사를 두 개로 나눈다.
- 메인 화면의 버튼을 모두 한글로 바꾼다.
