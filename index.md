...
### 26) 30주 퀘스트 및 아이콘 작업 완료
**추가 사항**
- 사용 및 파괴, 모두 얻기에 단축키 추가 ( 각각 A,S,Q )
- 해금되지 않은 아이템은 조합법에서 얻지 못하는 재료가 있는 아이템이라고 안내

**변경 사항**

- 실렌티움 공격력 60 -> 55, 공격력 강화율 6 -> 5
- 포이즌 이그니스 공격력 50 -> 60, 공격력 강화율 5 -> 6
- 골궁 공격력 40 -> 50, 공격력 강화율 4 -> 5
- 오래 전 용사의 견갑 조합법 변경
- 퍼루브타 테네브리스 조합법 변경
- 글라디우스 A Wonder -> A Unique / 공격력 58 -> 공격력 63 / 강몽석 비용 8 -> 9
- 누군가의 반지 R Wonder -> R Unique / 반격 보정 25% -> 28% / 크리티컬 확률 8% -> 10% / 반격 보정 확률 강화율 2% -> 3% 강몽석 비용 12 -> 13
- 괴기를 품은 두개골 S Wonder -> R Unique / 크리티컬 확률 26% -> 23%, 크리티컬 데미지 50% -> 80%
- 투지의 방패 R Unique -> S Wonder / 공격력 50 -> 52, 방어력 15 -> 20 / 방어력 강화율 1 -> 2
- 오래 전 영웅의 견갑 S Unique -> SR Wonder
- 거목의 뿌리 S Wonder -> R Unique / 공격력 75 -> 60, 크리티컬 확률 12% -> 10% / 공격력 강화율 7 -> 6
- 예리한 울프 스워드 공격력 75 -> 79 / 공격력 강화율 7 -> 8
- 오래 전 대장장이의 주괴 R Unique -> S Unique / 크리티컬 데미지 165% -> 260%, 크리티컬 확률 15% -> 12% / 크뎀 강화율 21% -> 32% 
- 피프스 애뮬렛 R Unique -> S Unique / 반격 보정 28% -> 32%, 강몽석 비용 13 -> 15
- 그로겔 이어링 반격 보정 32% -> 28%
- 원혼이 담긴 반지 반격 보정 35% -> 30%
- 검은 반지 반격 보정 40% -> 43%
- 투지의 보석 아니모 반격 보정 18% -> 20%, 크리티컬 데미지 45% -> 55% / 크리티컬 데미지 강화율 5% -> 7%
- 크레센트 링 반격 보정 30% -> 28%
- 두스 반격 보정 33% -> 30%
- 포레스트 링 반격 보정 32% -> 28%
- 루비 반지 반격 보정 22% -> 21%
- 보상 모두 받기 단축키가 생깁니다. ( Q로 조작이 가능합니다. )
- 완료 버튼 단축키가 Space에서 W로 변경
- 얻지 못하는 아이템은 자동으로 관심에서 빠지도록 수정
- 전투 UI 텍스트가 10줄이 넘으면 맨 위의 텍스트가 삭제되도록 변경
- 소형 포션이 2챕터 이상에서는 얻을 수 없는 아이템이라고 뜨는 버그 수정
- 마녀의 이미지가 변경
- 가방이 화면 중앙에서 나오도록 변경
- 가방 아이템 착용 버그 수정
- 해상도 개선 1차 작업
- 조합법에 재료 아이템이 어디서 나오는지 표기
- 잠긴 장비 아이템이 드래그하면 장착되는 버그 수정
- 괴기를 품은 두개골 강화 수치가 이상하던 버그 수정
- 일부 UI 디자인 변경
