...
### 24) 34주 음량 조절 시스템, 로딩 중 팁 추가 및 밸런스 조절
...
- ~~사전에 있는 아이템 칸을 클릭하면 아이템 설명 팝업창을 출력했다. 해당 아이템 설명 팝업창은 다른 팝업창과 동일한 크기를 가진다. (100%)~~
- ~~아이템 설명 팝업창의 우측 상단에는 팝업창을 끄는 버튼이 놓여져 있다. 그리고 그 중앙에는 아이템의 ID가 적혀져 있으며, 그 양옆에는 이전과 다음 아이템으로 넘어갈 수 있는 버튼이 놓여져 있다. (100%)~~
- ~~아이템 설명 팝업창의 좌측에는 위에서부터 아래로 아이템의 이름, 아이템의 이미지, 아이템의 등급, 그리고 아이템의 태그가 표시된다. (100%)~~
...
### 26) 36주 몹 사전 시스템 제작 및 피드백 반영
- ( 피드백 )6챕터에도 상자가 나오게 변경, 이때 상자를 조우할 확률은 7%다.
- ( 피드백 )6챕터의 모든 무기와 방어구, 그리고 데몬 넥클레스를 상향시킨다.
- ( 피드백 )끈기의 체력 추가 증가량을 25로 낮추고, 진행하기를 누를 때마다 체력이 3이 회복되게 리워크한다.
- ( 피드백 )아이템을 더블 클릭하면 사용되게 변경한다.
- ( 오류 )피격 중간에 게임을 끄면 피격 판정 자체가 일어나지 않은 취급이 되는 버그를 수정한다.
- 5챕터의 몬스터들에게서 나오는 기력의 간의 확률을 45%에서 50%로 증가시킨다.
- 도끼와 철 갑옷이 1 스테이지 상자에서 10%로 나오게 변경
- 타격 안내에 따라서 텍스트의 색깔이 변경되도록 수정 ( 플레이어의 타격 : 흰색, 플레이어의 치명타 : 파란색, 몬스터의 타격 : 주황색, 몬스터의 치명타 타격 : 빨간색, 플레이어의 데미지 0 : 초록색 )
- 한 챕터의 최종 스테이지 돌입 전에 플레이어에게 다음 스테이지가 확정 몬스터라는 것과 그 몬스터가 무엇인지 붉은 텍스트로 표시되게 변경
- 사전 팝업창 오른쪽에 현재 플레이어가 해금한 아이템 혹은 몬스터가 얼마나 되는지 상하 슬라이더로 표시하며, 슬라이더 위에는 현재 해금한 몬스터 혹은 아이템과 전체 갯수, 그리고 진행도를 표시한다.
- 홈 화면에 사전 버튼을 추가한다. 사전 버튼은 게임 시작 버튼 옆에 100x100 크기로 둔다.
- 몬스터 사전에 몬스터 아이콘이 표시되는 칸 아래에는 해당 몬스터의 ID와 그 몬스터의 이름이 표시된다. ( 텍스트의 크기는 자동으로 조정 )
- 몬스터 사전은 한 장에 최대 15개의 몬스터가 표시가 되며, 각각 1~11장으로 구분해놓는다.
- json 파일로 플레이어가 몬스터를 처치하면 그 몬스터의 ID에 해당하는 ID 변수를 true로 바꾼다. ( 초기값은 false )
- 플레이어의 json 파일에서 해당 몬스터의 ID와 같은 ID의 bool 변수가 false로 되어 있으면, 사전에 표시되는 몬스터의 아이콘이 검은색으로 출력되며, 이름, 체력, 공격력, 반격 확률, 치명타 반격 확률 또한 ???로 표시된다.
- 플레이어의 json 파일에서 해당 몬스터의 ID와 같은 ID의 bool 변수가 false로 되어 있으면, 사전에 표시되는 설명은 모두 ???로 출력되며, 출저가 모두 출력되지 않는다.
- 플레이어의 json 파일에서 해당 몬스터의 ID와 같은 ID의 bool 변수가 true로 되어 있으면, 사전에 표시되는 몬스터의 아이콘이 정상적으로 출력되며, 이름 또한 정상적으로 표시된다.
- 사전에 있는 몬스터 칸을 클릭하면 몬스터 설명 팝업창을 출력했다. 해당 몬스터 설명 팝업창은 다른 팝업창과 동일한 크기를 가진다.
- 몬스터 설명 팝업창의 우측 상단에는 팝업창을 끄는 버튼이 놓여져 있다. 그리고 그 중앙에는 몬스터의 ID가 적혀져 있으며, 그 양옆에는 이전과 다음 몬스터으로 넘어갈 수 있는 버튼이 놓여져 있다.
- 몬스터 설명 팝업창의 좌측에는 위에서부터 아래로 몬스터의 이름, 몬스터의 이미지, 몬스터의 체력, 몬스터의 공격력, 몬스터의 반격 확률, 몬스터의 치명타 반격 확률이 표시된다.
- 몬스터 설명 팝업창의 중앙에는 해당 몬스터의 보스 여부와 설명이 위에서 아래로 차례로 출력된다.
- 몬스터 설명 팝업창의 우측에는 해당 몬스터에게서 어떤 아이템을 얻을 수 있는지, 어떤 챕터에서 나오는지 출력된다.
- 몬스터 설명 팝업창의 우측에 있는 아이템 안내는 32x32 크기의 이미지 칸으로 표시하며, 그 칸의 우측에는 그 아이템의 이름이 출력이 된다.
...
마지막으로 BGM를 추가해서 베타 버전 마무리 작업에 들어갈 예정이다.
