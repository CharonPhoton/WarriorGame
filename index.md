...
**추가 사항**
- 자동 장착 시스템을 추가한다.
- 자동 장착 시스템이 활성화된 상태에서 장비가 파괴되면 그 장비 아이템 이하의 내구도 수치를 가진 아이템을 자동으로 장착한다.
- 만약에 내구도 수치 이하의 아이템이 없을 경우, 내구도 수치를 초과하는 아이템을 자동으로 장착한다.
- 아이템을 잠그는 시스템을 추가한다. 잠겨진 아이템은 붉게 변하며, 사용 및 삭제가 불가능해진다.

**변경 사항**

- 모든 UI로 도트로 변경한다. ( 단, 아이템은 아직 되지 않았으니 이후에 한다. )
- 상점의 아이템 정보 UI를 구입 팝업창과 통합시킨다.
- 아이템의 마나 필요 표시는 사용 버튼의 텍스트가 변경되는 방식으로 변경한다.
- 몬스터 등장 연출 버튼을 삭제한다.
- 여정의 옵션에 있는 자동 장착 버튼, 인벤-가방 아이템 보상 수령 방식 버튼, 아이템 수령 경고 버튼을 바깥으로 옮긴다.
- 어둠 속에 존재하는 자와의 패배로 재몽석을 사용할 경우, 포기하기를 누르면 게임에서 패배하는 버그를 수정한다.
- 아이템 설명 부분 오타가 있는 것을 수정한다.
- PC에서 Space 버튼으로 공격 및 진행하기 버튼이 눌려지도록 변경한다.
- 스킵한 이후, 대화가 다시 출력되면 스킵 팝업창이 켜지던 버그를 수정한다.
- 치환 특성이 상점에서 충전이라는 이름으로 되어 있던 버그 수정한다.
