...
- 일단 아이템과 특성 강화는 한 번만 가능하며, 아이템 강화는 가죽 보호대를 제외한 모두 3 강몽석이 요구된다. 가죽 보호대는 4 강몽석이 요구되며, 특성은 모두 10 강몽석이 요구된다.
- 일단 아이템과 특성 강화의 첫 강화 확률은 모두 100%다.
...
- 모든 씬에서 출력되는 아이템과 특성( 인게임에서 화면 중앙에 표시되는 특성 이름, 몬스터 사전에 표기된 드랍되는 아이템들의 이름 제외 )은 만약에 강화된 상태일 경우, 그 이름에 (" + 강화 수치 ")를 붙이며, 효과는 현재 강화된 아이템 또는 특성의 수치로 표기한다.
