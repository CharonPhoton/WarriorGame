...
### 4) 4주 히든 보스 및 등장 연출 추가
**추가 사항**
- 보스 몬스터들의 대사를 추가한다. 대사는 몬스터와 첫 조우 할 경우, 몬스터를 쓰러뜨렸을 경우, 몬스터가 드랍하는 에픽 아이템을 장착했을 때 조우했을 경우 출력되게 한다.
- 단, 최종보스인 마왕은 조우할 경우에만 출력되게 만들며, 히든 보스인 어둠 속에 존재하는 자는 추가하지 않는다.
- 상점에서 해금할 수 있는 아이템을 추가한다.
- 상점 해금 아이템인 트리플 슛을 추가한다. 트리플 슛은 본웜에게서 나오는 A등급 Rare 마법 아이템으로, 적에게 145 데미지를 주는 효과가 있다.
- 상점 해금 아이템인 마그나 플레마 익스플로전을 추가한다. 마그나 플레마 익스플로전은 크리스털 스파이더에게서 나오는 S등급 Wonder 마법 아이템으로, 적에게 280 데미지를 주고 화상 3단계 상태이상을 남기는 효과가 있다.
- 상점 해금 아이템인 블랙 슬라임의 원념을 추가한다. 블랙 슬라임의 원념은 블랙 슬라임에게서 나오는 반격 보정 22% 증가시키는 B등급 Unique 장신구 아이템이다.
- 상점 해금 아이템은 루인드 아머를 추가한다. 루인드 아머는 라스 웜에게서 나오는 방어력 49, 공격력 15, 크리티컬 데미지가 70% 증가하는 R등급 Wonder 방어구 아이템이다.
- 새로운 조합 아이템 퍼루브타 테네브레스를 추가한다. 페루브타 테네브레스는 SR등급 Wonder 단검 무기 아이템으로, 공격력 80, 반격 보정 10%를 증가해준다. ( 조합법은 단검 + 슬래셔 + 젬 + 쌍검 베나리&세미타 + 수르쿨루스 )
- 가방을 제외한 B등급 아이템을 일단 4단계까지 강화 가능하게 바꾼다. ( 단, 집중의 물약은 1단계, 스팀팩은 2단계까지. )
- 상점에서 여러 개 아이템을 구입하는 창에 최대치까지 설정할 수 있는 버튼을 증가 버튼 오른쪽에 추가한다.

**변경 사항**

- 수크쿨루스 등급 B에서 C로 하향, 공격력 23에서 21로, 크리티컬 데미지가 55%에서 40%로 하향
- 마스터 케로베로스의 인내의 공격력이 75에서 78로 상향
- 투지의 방패의 공격력이 45에서 47로 상향
- 코팅 쉴드의 방어력 증가 강화 가격이 9에서 8로 감소
- 버섯인간 스포아의 공격력이 24에서 26으로 증가
- 매직 스태프의 최종 강화 추가 공격력이 4에서 5로 상향
- 투지의 방패의 내구도가 1 낮았던 버그를 수정한다.
- ????의 설명을 수정한다.
- 해골전사 켈트의 스토리 설명을 수정한다.
- 무속성 마법 아이템의 공격력 표기가 강화 수치에 따라서 변하지 않는 버그를 수정한다.
- 강몽석이 인벤토리가 가득 찰 경우 얻을 수 없던 버그를 수정한다.
