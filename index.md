...
- Act 4에 사용되는 챕터 6의 배경화면을 추가한다.
- Act 4의 최종 보스에게 사용되는 스프라이트를 추가한다.
- Act 4의 모든 대화집을 영문화한다.
- 아이템 일괄 강화 시스템을 추가한다. 아이템 일괄 강화 시스템은 필터에 걸러진 아이템들은 최소 강화부터 한 단계씩 강화할 수 있는 시스템이다.
- 홈 화면에 Act와 관련된 튜토리얼을 추가한다. ( Act 2 해금이 되면 출력이 된다. )

**변경 사항**

- 힘의 견갑의 방어력을 21에서 28로 상향하며, 방어력 강화율을 2에서 3으로 상향한다.
- 영어 점수 표기가 한국어로 나오던 버그를 수정한다.
- 포레스트 링이 엉뚱한 몬스터에게도 나오던 버그를 수정한다.
- 의지의 복구석 / 통찰의 복구석 / 한계의 복구석의 버그를 수정한다.
