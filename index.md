...
### 23) 27주 UI 변경
**추가 사항**
- 3가지 퀘스트를 추가한다. 퀘스트에는 스토리 퀘스트, 반복 퀘스트, 목표 퀘스트가 있다. (20%)
...
### 28) 32주 전시회 의견 반영 3차
**추가 사항**
- 비어있는 장비 칸에 빠르게 장비를 장착할 수 있는 키를 추가하며, 비어있는 장비 칸을 누르면 빠르게 장비를 장착할 수 있게 한다.
- 빠르게 장비를 장착할 경우에는 등급이 가장 낮고, 내구도가 가장 낮은 무기를 장착한다.
- 빠르게 장비를 장착하는 단축키는 무기, 방어구, 장신구, 부적 각각 Q,W,E,R로 설정한다.
- 능력치를 보는 UI는 버튼이나 C키를 눌러서 키거나 끌 수 있도록 변경하며, 위치는 기존 위치의 바로 좌측으로 옮긴다.
- 마법 아이템을 개별로 보관하는 인벤토리 창을 추가한다. 이때, 마법 아이템 인벤토리 칸은 8개이며, 기본 인벤토리 UI처럼 드래그 기능은 포함하지 않는다.
- 마법 아이템은 최대 세 개까지 중첩하여 모을 수가 있다.
- 마법 아이템을 얻을 때, 인벤토리 안에 보유 수가 2개 이하인 마법이 있으면, 해당 아이템에 중첩한다. 아닐 경우에는 비어있는 인벤토리 칸에 추가한다. 만약에, 마법 아이템이 모두 차면 Max로 표시한다.
- 마법 아이템은 기존처럼 사용할 수 있게 한다.
- 새로운 조합 아이템 소울 스태프를 추가한다. 소울 스태프는 S등급 Unique 스태프 무기 아이템으로, 공격력 80 증가해주며, 80% 확률로 1턴 동안 3단계 동결 상태이상을 부여한다. ( 조합법은 매직 스태프 + 원한의 해골 + 포이즌 스태프 + 폐허의 씨앗 )

**변경 사항**

- 모두 얻기, 완료 버튼이 Q/W에서 A/S로 변경한다
- 아이템 착용 삭제 버튼이 A/S에서 Z/X로 변경한다.
- 가방 최대 보유 가능 개수가 8개에서 7개로 변경한다.
- 옵션 창이 뜬 상태에서 스페이스 바를 누르면 작동이 되던 버그를 수정한다.
