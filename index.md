...
### 8) 8주 피드백 반영 편의성 패치
**추가 사항**
- 가방 팝업창 상단에 자동으로 넣어질 때 특정 아이템 종류만 넣을 수 있는 드랍박스를 만든다.
- 가방 팝업창 상단에 PC에서만 보이는 단축키 설정 드랍박스를 만든다.
- 가방 아이템 데이터에 특정 아이템 종류의 데이터를 추가해서 필터를 저장할 수 있게 만든다.
- 보상이 가방으로 자동으로 넣어질 때, 그 가방의 필터를 참조할 수 있도록 만든다.
- PC 버전에서는 가방 아이템 우측 상단에 해당 가방의 단축키 (1~8)을 표시한다.
- 가방 아이템의 최초 단축키는 그 아이템의 이름에서 Bag 이름을 뺀 형태이다. 만약에 해당 단축키를 이미 다른 가방 아이템이 사용하고 있을 경우, 단축키를 사용하지 않은 곳을 1부터 검색하여 설정한다.
- 가방 아이템의 단축키는 가방 팝업창 상단 좌측에 있는 드랍박스로 설정할 수 있다.
- 만약에 이미 설정된 단축키로 단축키를 변경할 경우, 두 가방의 단축키를 바꾸도록 설정한다.
- 게임 옵션에 인벤토리 -> 가방 순으로 아이템을 넣을지, 아니면 가방 -> 인벤토리 순으로 아이템을 넣을지 설정할 수 있는 버튼을 추가한다.
- 보상 중에 인벤토리에 넣지 못한 아이템이 중간에 있는 경우, 넣을 수 있는 아이템들을 모두 넣고 아이템을 넣을 수 없다는 경고문을 출력하도록 변경한다.
- 옵션 조합법에 관심 버튼을 추가하여 해당 아이템을 이후에 무조건 먼저 출력되도록 만드는 버튼을 추가한다. ( 이 버튼의 활성화는 투명도로 확인하게 한다. )
- 보스의 치명타 공격 / 치명타 피격 대사를 추가한다.
- 마법 모자를 제외한 R등급 아이템 강화를 추가한다.

**변경 사항**

- 보상 중에 인벤토리에 넣지 못한 아이템이 중간에 있는 경우, 넣을 수 있는 아이템들을 모두 넣고 아이템을 넣을 수 없다는 경고문을 출력하도록 변경한다.
- 방어력, 반격 보정 확률 증가 효과를 가진 소비 아이템의 강화 효과가 제대로 적용되지 않던 버그를 수정한다.
- 그린 슬라임이 10% 확률로 벨트를 드랍하도록 변경한다.
- 썬더 익스플로전의 공격력을 118에서 140으로 상향한다.
