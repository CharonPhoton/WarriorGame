...
- 인벤토리 칸의 소비 아이템이 아래에 어떤 효과를 가지고 있는지 표시되게 변경.( 능력치 증가는 Status, 내구도 수리는 Fix, 회복 아이템은 +이라는 글자 옆에 회복량을 표시한다. )
- 마법 아이템 타입을 추가한다. 특정 아이템은 자신이 마법 아이템이라고 표시한다.
- 새로운 희귀성인 Legend를 추가한다. Legend는 가장 높은 등급이며, 빨간색으로 표시된다.
- Legend 희귀성 아이템을 얻으면 그 아이템의 랭크 x 300의 점수를 얻는다.
- 무속성의 데미지를 주는 마법 아이템을 사용하면 타격 이펙트를 출력하게 한다.
- 마법 아이템으로 타격을 주면 마법에 의해 데미지를 받았다는 텍스트를 출력하게 한다.( 텍스트 색깔은 푸른색으로 뜬다. )
- 타격 이펙트의 색깔을 아이템의 등급의 색깔과 똑같은 색상으로 바꾼다.
- 새로운 마법 아이템을 6개를 추가한다.
- 매직 미사일은 챕터 1의 몬스터들에게 15% 확률로 나오며, 무속성의 데미지를 적에게 준다. 이때, 효과는 무언가에 타격을 받은 듯한 효과가 출력된다.
- 매직 미사일의 데미지는 25다.
- 매직 스워드은 챕터 2의 몬스터들에게 15% 확률로 나오며, 무속성의 데미지를 적에게 준다. 이때, 효과는 무언가에 베이는 듯한 효과가 출력된다.
- 매직 스워드의 데미지는 45다.
