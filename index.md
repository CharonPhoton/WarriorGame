...
- ~~어둠 속에 존재하는 자는 체력이 4,000이며, 공격력이 53이며, 반격 확률이 70%이며, 치명타 확률이 38%다. (100%)~~
...
- ~~어둠 속에 존재하는 자를 처음 조우할 때의 대화를 추가한다. (100%)~~
- ~~어둠 속에 존재하는 자를 처치했을 때의 대화를 추가한다. (100%)~~
- ~~어둠 속에 존재하는 자에게 졌을 때의 대화를 추가한다. (100%)~~
- ~~추가한 대화집은 각각 상황에 맞는 곳에 출력하도록 하며, 이 대화집은 csv 파일을 통해 관리하도록 한다. (100%)~~
- ~~고대석, 블러드 해프닝, 쇼크 브레이크를 제외한 C등급 아이템를 최대 4번까지 강화할 수 있도록 만든다. ( 이때, 힘의 물약, 인내의 물약, 괴력육은 한 번만 강화가 가능하도록 변경한다. ) (100%)~~
...
- ~~들쥐 부적의 크리티컬 데미지 추가 효과가 100%에서 120%로 상향된다. (100%)~~

### 5) 5주 보스 대사 추가
**추가 사항**
- ~~보스 몬스터들의 대사를 추가한다. 대사는 몬스터와 첫 조우 할 경우, 몬스터를 쓰러뜨렸을 경우, 몬스터가 드랍하는 에픽 아이템을 장착했을 때 조우했을 경우 출력되게 한다. (100%)~~
- ~~단, 최종보스인 마왕은 조우할 경우에만 출력되게 만들며, 히든 보스인 어둠 속에 존재하는 자는 추가하지 않는다. (100%)~~
- ~~상점에서 해금할 수 있는 아이템을 추가한다. (100%)~~
- ~~상점 해금 아이템인 트리플 슛을 추가한다. 트리플 슛은 본웜에게서 나오는 A등급 Rare 마법 아이템으로, 적에게 145 데미지를 주는 효과가 있다. (100%)~~
- ~~상점 해금 아이템인 마그나 플레마 익스플로전을 추가한다. 마그나 플레마 익스플로전은 크리스털 스파이더에게서 나오는 S등급 Wonder 마법 아이템으로, 적에게 280 데미지를 주고 화상 3단계 상태이상을 남기는 효과가 있다. (100%)~~
- ~~상점 해금 아이템인 블랙 슬라임의 원념을 추가한다. 블랙 슬라임의 원념은 블랙 슬라임에게서 나오는 반격 보정 22% 증가시키는 B등급 Unique 장신구 아이템이다. (100%)~~
- ~~상점 해금 아이템은 루인드 아머를 추가한다. 루인드 아머는 라스 웜에게서 나오는 방어력 49, 공격력 15, 크리티컬 데미지가 70% 증가하는 R등급 Wonder 방어구 아이템이다. (100%)~~
- ~~새로운 조합 아이템 퍼루브타 테네브레스를 추가한다. 페루브타 테네브레스는 SR등급 Wonder 단검 무기 아이템으로, 공격력 80, 반격 보정 10%를 증가해준다. ( 조합법은 단검 + 슬래셔 + 젬 + 쌍검 베나리&세미타 + 수르쿨루스 ) (100%)~~
- ~~가방을 제외한 B등급 아이템을 일단 4단계까지 강화 가능하게 바꾼다. ( 단, 집중의 물약은 1단계, 스팀팩은 2단계까지. ) (100%)~~
- ~~상점에서 여러 개 아이템을 구입하는 창에 최대치까지 설정할 수 있는 버튼을 증가 버튼 오른쪽에 추가한다. (100%)~~

**변경 사항**

- ~~수크쿨루스 등급 B에서 C로 하향, 공격력 23에서 21로, 크리티컬 데미지가 55%에서 40%로 하향 (100%)~~
- ~~마스터 케로베로스의 인내의 공격력이 75에서 78로 상향 (100%)~~
- ~~투지의 방패의 공격력이 45에서 47로 상향 (100%)~~
- ~~코팅 쉴드의 방어력 증가 강화 가격이 9에서 8로 감소 (100%)~~
- ~~버섯인간 스포아의 공격력이 24에서 26으로 증가 (100%)~~
- ~~매직 스태프의 최종 강화 추가 공격력이 4에서 5로 상향 (100%)~~
- ~~투지의 방패의 내구도가 1 낮았던 버그를 수정한다. (100%)~~
- ~~????의 설명을 수정한다. (100%)~~
- ~~해골전사 켈트의 스토리 설명을 수정한다. (100%)~~
- ~~무속성 마법 아이템의 공격력 표기가 강화 수치에 따라서 변하지 않는 버그를 수정한다. (100%)~~
- ~~강몽석이 인벤토리가 가득 찰 경우 얻을 수 없던 버그를 수정한다. (100%)~~
  
### 6) 6주 튜토리얼 대사 추가, 사전 개편
**추가 사항**
- ~~사전 씬을 별도로 만든다. 사전 씬에서 아이템 사전과 몬스터 사전, 그리고 게임의 대화집을 볼 수 있게 만든다. (100%)~~
- ~~왼쪽에 있는 바에 위에서부터 아이템 사전 버튼, 몬스터 사전 버튼, 그리고 캐릭터들의 대화집 버튼을 목차로 둬서 추가한다. (100%)~~
- ~~아이템 사전 버튼과 몬스터 사전 버튼을 누르면 이전의 화면이 우측 하단의 거대한 배경에 출력되게 만든다. ( 단, 두루마리 위에 출력되게 만드며, 우측 상단의 바에 필터 버튼을 둔다. ) (100%)~~
- ~~목차의 대화집 버튼을 누르면 우측 상단의 바에 상황에 맞는 대화집 버튼을 추가한다. (100%)~~
- ~~목차의 대화집 버튼에 왼쪽에는 대표 이미지와 오른쪽에는 캐릭터의 이름을 추가한다. ( 단, 튜토리얼은 튜토리얼로 하고 왼쪽에는 플레이어(용사)의 사진을 둔다. ) (100%)~~
- ~~아이템 사전과 몬스터 사전은 홈 화면에 있던 버튼의 이미지를 사용한다. (100%)~~
- ~~우측 상단의 상황 대화집 버튼을 누르면 우측 하단의 배경에 인게임에서의 대화집의 화면을 똑같이 구현해서 대화를 출력한다. ( 단, 마지막 대화 다음에는 첫 대화 장면으로 돌아오게 만든다. ) (100%)~~
- ~~사전에 있는 요소들은 해금이 되면 몽환석을 얻을 수 있게 만든다. 이때, 요소들은 아이템, 몬스터, 그리고 상황에 따른 대화집이 있으며, 해당 버튼 위에 몽환석을 얻지 않은 상태면 몽환석 이미지를 출력시키게 만든다. ( 배경은 검게 만든다. ) (100%)~~
- ~~몽환석을 얻은 상태의 요소들은 몽환석 이미지가 출력되지 않으며, 누르면 본래의 기능이 작동된다. ( 몽환석이 떠 있는 상태에서 누르면 기능이 작동되지 않고 몽환석을 얻게 만든다. ) (100%)~~
- ~~아이템 요소는 몽환석 20, 몬스터 요소는 몽환석 25, 대화집 요소는 몽환석 30을 제공한다. (100%)~~
- ~~우측 상단의 주머니 버튼을 눌러서 현재 출력된 요소의 몽환석들을 한꺼번에 수거할 수 있도록 만든다. ( 이때, 정말 수거할 거냐고 물으며, 수거하면 얻을 수 있는 몽환석 개수를 출력한다. ) (100%)~~
- ~~조합법에 아이템 필터링을 추가한다. (100%)~~
- ~~새로운 특성인 첫발을 추가한다. 첫발은 새로운 챕터에 도달하면 일정 스테이지까지 유물 조우 확률을 2.3배 높여준다. (100%)~~
- ~~첫발의 유물 조우 확률 증가 효과는 노말 속도에서는 3 스테이지까지, 패스트 속도에서는 2 스테이지까지, 베리 패스트 속도에서는 1 스테이지까지 적용된다. (100%)~~
- ~~금강의 강화가 가능하도록 변경한다. ( 강화 시 치명타 피해 감소량 80% -> 75% ) (100%)~~
- ~~튜토리얼 독백 장면을 추가한다. 일정 상황에 직면하면 독백을 하여 해당 상황을 설명하는 방식이다. (100%)~~
- ~~튜토리얼 독백은 프롤로그 닫을 경우, 몬스터를 조우한 경우, 유물을 조우한 경우, 치유의 숲을 조우한 경우, 이상한 장소를 조우한 경우, 수상한 상자를 조우한 경우, 종류 별 아이템을 얻었을 경우, 내구도를 감소한 경우, 보스 경고가 처음 뜬 경우이다. (100%)~~

**변경 사항**

- ~~탐색의 설명에 있는 상자 단어를 유품으로 변경 (100%)~~
- ~~강몽석이 인벤토리가 꽉 찰 경우, 모두 얻기를 하면 얻을 수 없던 버그를 수정 (100%)~~

### 7) 7주 본격 스토리 추가
**추가 사항**
- ~~해당 상황에서의 대화를 추가한다. ( 상점에 있는 마녀는 가능한 대화가 있다면 마녀를 누르면 대화 목록을 뜨게 만든다. ) (100%)~~
- ~~다음 상황에서의 대화를 추가한다. ( 엔딩 이후 홈 화면, 엔딩 이후 마녀 대화, 어둠 속에 존재하는 자 대면 이후 마녀 대화, 너클 종류 무기 획득 후 마녀 대화, 강몽석과 재몽석 마녀 대화, 여정과 상점 마녀 대화, 이상한 공간 마녀 대화, 치유의 숲 마녀 대화 ) (100%)~~
- ~~복구석을 추가한다. 복구석은 최근에 파괴된 특정 종류의 아이템을 부활시킬 수가 있다. 복구석은 챕터 4~6의 몬스터에게서 나오는 S등급 에픽 아이템이다. (100%)~~
- ~~긍지의 복구석은 최근에 파괴된 무기 아이템을 내구도를 50%로 하여 복구시킨다. 긍지의 복구석은 챕터 4~6에서 나오는 첫 번째 몬스터에게서 3% 확률로 나온다. (100%)~~
- ~~의지의 복구석은 최근에 파괴된 방어구 아이템을 내구도를 50%로 하여 복구시킨다. 의지의 복구석은 챕터 4~6에서 나오는 두 번째 몬스터에게서 3% 확률로 나온다. (100%)~~
- ~~통찰의 복구석은 최근에 파괴된 장신구 아이템을 내구도를 50%로 하여 복구시킨다. 통찰의 복구석은 챕터 4~6에서 나오는 세 번째 몬스터에게서 3% 확률로 나온다. (100%)~~
- ~~한계의 복구석은 최근에 파괴된 부적 아이템을 내구도를 50%로 하여 복구시킨다. 한계의 복구석은 챕터 4~6에서 나오는 네 번째 몬스터에게서 3% 확률로 나온다. (100%)~~
- ~~새로운 아이템인 ?? ?????를 추가한다. ?? ?????은 R등급 Legend 너클 종류의 무기 아이템으로 공격력 60, 방어력 17, 반격 보정 확률 7%, 크리티컬 데미지 100%, 크리티컬 확률 7%를 제공한다. 스포아, 바드, 켈트에게서 1% 확률로 나온다. (100%)~~
- ~~새로운 특성인 육중을 추가한다. 육중은 체력을 120 제공하는 특성으로, 강화하면 180 체력을 제공한다. (100%)~~
- ~~고대석을 제외한 A등급 아이템의 강화를 4단계까지 가능하게 한다.  (100%)~~

**변경 사항**

- ~~끈기 + 1의 체력 증가량이 50에서 40으로 감소한다. (100%)~~
- ~~집중의 물약의 가격을 8에서 12으로 증가한다. (100%)~~
- ~~보스 경고 튜토리얼이 2 챕터 이후부터 나오도록 수정한다. (100%)~~
- ~~사전의 대화집에서 플레이어라는 이름을 용사로 수정한다. (100%)~~
- ~~대화가 겹치면 차례로 출력되도록 수정한다. (100%)~~
- ~~사전이 제대로 작동되지 않던 버그를 수정한다. (100%)~~

### 8) 8주 피드백 반영 편의성 패치
**추가 사항**
- ~~가방 팝업창 상단에 자동으로 넣어질 때 특정 아이템 종류만 넣을 수 있는 드랍박스를 만든다. (100%)~~
- ~~가방 팝업창 상단에 PC에서만 보이는 단축키 설정 드랍박스를 만든다. (100%)~~
- ~~가방 아이템 데이터에 특정 아이템 종류의 데이터를 추가해서 필터를 저장할 수 있게 만든다. (100%)~~
- ~~보상이 가방으로 자동으로 넣어질 때, 그 가방의 필터를 참조할 수 있도록 만든다. (100%)~~
- ~~PC 버전에서는 가방 아이템 우측 상단에 해당 가방의 단축키 (1~8)을 표시한다. (100%)~~
- ~~가방 아이템의 최초 단축키는 그 아이템의 이름에서 Bag 이름을 뺀 형태이다. 만약에 해당 단축키를 이미 다른 가방 아이템이 사용하고 있을 경우, 단축키를 사용하지 않은 곳을 1부터 검색하여 설정한다. (100%)~~
- ~~가방 아이템의 단축키는 가방 팝업창 상단 좌측에 있는 드랍박스로 설정할 수 있다. (100%)~~
- ~~만약에 이미 설정된 단축키로 단축키를 변경할 경우, 두 가방의 단축키를 바꾸도록 설정한다. (100%)~~
- ~~게임 옵션에 인벤토리 -> 가방 순으로 아이템을 넣을지, 아니면 가방 -> 인벤토리 순으로 아이템을 넣을지 설정할 수 있는 버튼을 추가한다. (100%)~~
- ~~옵션 조합법에 관심 버튼을 추가하여 해당 아이템을 이후에 무조건 먼저 출력되도록 만드는 버튼을 추가한다. ( 이 버튼의 활성화는 투명도로 확인하게 한다. ) (100%)~~
- ~~보스의 치명타 공격 / 치명타 피격 대사를 추가한다. (100%)~~
- ~~마법 모자를 제외한 R등급 아이템 강화를 추가한다. (100%)~~

**변경 사항**

- ~~보상 중에 인벤토리에 넣지 못한 아이템이 중간에 있는 경우, 넣을 수 있는 아이템들을 모두 넣고 아이템을 넣을 수 없다는 경고문을 출력하도록 변경한다. (100%)~~
- ~~방어력, 반격 보정 확률 증가 효과를 가진 소비 아이템의 강화 효과가 제대로 적용되지 않던 버그를 수정한다. (100%)~~
- ~~그린 슬라임이 10% 확률로 벨트를 드랍하도록 변경한다. (100%)~~
- ~~썬더 익스플로전의 공격력을 118에서 140으로 상향한다. (100%)~~

### 9) 9~10주 Act 2 패치
**추가 사항**
- ~~Act 2를 추가한다. Act 2는 Act 1과 유사하지만 몬스터들의 능력치가 더 강화된 상태며, 상자 조우 확률이 2% 낮고, 상기 상자와 치유의 숲의 가격이 증가하는 특징이 있다. (100%)~~
- ~~Act 2의 몬스터는 모두 기존 몬스터의 체력 110% / 반격 확률 95% / 치명타 반격 확률 95%의 능력치를 가지고 있는 특징이 있다. (100%)~~
- ~~Act 2에서는 기존의 점수를 110%의 배율로 받을 수 있다. (100%)~~
- ~~Act 1에서 2챕터부터 해당 챕터에 도달하면 스토리 대사가 나오도록 출력한다. (100%)~~
- ~~엔딩에서 만약에 엔딩 대사 연출을 보지 않은 상태라면 대사를 출력하고 마지막으로 게임 결과 화면을 출력하게 만든다. (100%)~~
- ~~챕터 도달 튜토리얼, Act 1 엔딩, Act 2 스토리, 마녀 대화를 추가한다. ( 스토리와 대사는 예정된 것처럼 ) (100%)~~
- ~~Act 2의 마녀 대화까지 마치면 홈 화면이 더 어두워지며, BGM이 바뀌도록 한다. (100%)~~
- ~~Act 1 엔딩에서는 챕터 1의 들판이, Act 2 엔딩에서는 치유의 숲과 재몽 석상의 배경 화면이 나오도록 만든다. (100%)~~

**변경 사항**

- ~~독 바른 검의 공격력이 25에서 28로 상향, 독 바른 검의 강화 상승량이 2에서 3으로 상향 (100%)~~
- ~~코팅 실드의 공격력이 16에서 19로 상향, 코팅 실드의 무기 공격력 강화 상승량이 1에서 2로 상향 (100%)~~
- ~~아이언 링이 강화가 되면 크리티컬 데미지도 함께 오르도록 변경 (100%)~~
- ~~챕터 1 벨트의 등장 확률을 3% 증가 (100%)~~
- ~~생명의 기억이 체력을 50이 아닌 60을 올려주는 것으로 상향 (100%)~~
- ~~포이즌 웜의 반격 확률 40%에서 35% 낮아지며, 치명타 반격 확률이 25%에서 22로 낮아집니다. (100%)~~
- ~~어둠 속에 존재하는 자도 강몽석이 나오도록 변경 ( 단, 확률은 다른 몬스터들과 다르게 50% ) (100%)~~
- ~~재몽석 가격 5000에서 4000으로 하향 (100%)~~
- ~~점수로 인한 강몽석 획득 조건이 점수 / 75,000에서 점수 / 60,000으로 상향 (100%)~~
- ~~철 갑옷 강화가 누락되었던 버그 수정 (100%)~~
- ~~프로테가트 스토리 설명 오타 수정 (100%)~~
- ~~대화 넘어갈 때 나는 소리가 배경화면 음악 크기에 영향을 받던 버그 수정 (100%)~~
- ~~Act. 1 엔딩 이후 대화가 변경 (100%)~~
- ~~Act. 1 엔딩 이후 마녀와의 대화가 사전에서 스토리에 들어가도록 변경 (100%)~~
- ~~이상한 공간에 대한 대화가 변경 (100%)~~
- ~~연출 끄기 상태에서 보스를 조우하면 대사가 안 뜨던 버그가 수정 (100%)~~
- ~~프롤로그 오타 수정 (100%)~~

### 10) 11주 밸런스 및 추가 패치
**추가 사항**
- ~~새로운 조합 아이템 거목의 뿌리를 추가한다. 거목의 뿌리는 S등급 Wonder 활 무기 아이템으로, 공격력 75, 크리티컬 확률 12%를 증가해준다. ( 조합법은 롱보우 + 아머드 보우 + 세계수 활 ) (100%)~~
- ~~새로운 조합 아이템 오래 전 대장장이의 주괴를 추가한다. 오래 전 대장장이의 주괴는 R등급 Unique 부적 아이템으로, 크리티컬 피해 165%, 크리티컬 확률 15%를 증가해준다. ( 조합법은 은 주괴 + 마골 + 금괴 + 붉은 마법책 ) (100%)~~
- ~~상점에서 해금할 수 있는 아이템을 네 가지 추가한다. (100%)~~
- ~~상점 해금 아이템인 아케인 스태프를 추가한다. 아케인 스태프는 포이즌 그로겔에게서 나오는 공격력 38 증가, 60% 확률로 1턴 동안 동결 2단계 상태이상을 부여하는 A등급 Wonder 지팡이 무기 아이템이다. (100%)~~
- ~~상점 해금 아이템인 사이트 라이플을 추가한다. 사이트 라이플은 라바 슬라임에게서 나오는 공격력 34, 크리티컬 피해를 70% 증가시키는 A등급 Rare 총 무기 아이템이다. (100%)~~
- ~~상점 해금 아이템인 노블 케이프를 추가한다. 노블 케이프는 다크니스 그로겔에게서 나오는 방어력 64, 공격력 9 증가시키는 S등급 Wonder 방어구 아이템이다. (100%)~~
- ~~상점 해금 아이템인 포레스트 링을 추가한다. 포레스트 링은 본웜에게서 나오는 반격 보정 32%, 방어력 10 증가시키는 R등급 Unique 장신구 아이템이다. (100%)~~
- 서버 연동 시스템을 만든다. (60%)
- ~~이메일을 입력하여 해당 이메일로 인증번호로 받고, 이메일 / 비밀번호 / 인증번호를 입력하면 회원가입이 되도록 만든다. ( 만약 이미 회원가입이 된 계정이면 팝업창을 띄워서 이미 가입된 이메일이라고 표시한다. ) (100%)~~
- ~~회원가입에 성공할 경우, 플레이어가 가지고 있는 데이터를 모두 서버에 저장하도록 만든다. (100%)~~
- ~~플레이어가 이미 이전에 로그인을 했던 상태일 경우, 게임이 가동될 시에 토큰으로 자동으로 로그인이 되도록 만든다. (100%)~~
- ~~플레이어가 로그아웃을 하면 입력창을 초기화하고, 완벽하게 로그아웃 시킨다. (100%)~~
- ~~플레이어가 로그인인 상태로 로그인 창을 열면 세이브, 로드, 로그아웃 버튼이 나오도록 만든다. (100%)~~
- ~~플레이어가 로그아웃인 상태로 로그인 창을 열면 이메일, 비밀번호 입력창, 로그인과 회원가입 버튼이 나오도록 만든다. (100%)~~
- ~~회원가입 버튼을 누르면, 로그인 버튼이 있는 곳에 인증번호를 입력할 수 있는 입력창이 나오며, 이메일 옆에는 인증번호를 해당 이메일로 보낼 수 있는 버튼이 있다. (100%)~~
- ~~플레이어가 만약에 인터넷에 연결된 상태가 아니라면 로그인 창을 열 수 있는 버튼을 비활성화시킨다. (100%)~~
- ~~플레이어가 이미 저장을 한 상태라면 전역 변수로 변수를 false로 만들고, 서버 데이터 저장과 로드 버튼을 비활성화 시킨다. (100%)~~
- ~~이후에 데이터에 변화가 있을 경우 이 변수를 true로 바꿔서 서버 데이터 저장과 로드 버튼을 활성화 시킨다. (100%)~~
- ~~플레이어의 레코드 데이터에 변화가 있을 때, 그리고 처음 회원가입을 했을 때 노말, 스피드, 베리 스피드에서 가장 높은 랭킹의 데이터를 서버로 전송한다. (100%)~~
- ~~홈 화면에 플레이어가 서버 랭킹 20위까지의 데이터를 볼 수 있는 창을 열 수 있는 버튼을 추가한다. (100%)~~

**변경 사항**

- ~~비르투스 네클레스의 반격 보정 효과가 18%에서 16%로, 방어력 증가 효과가 12에서 8로 감소 (100%)~~
- ~~비르투스 네클레스의 반격 보정 강화율이 2%에서 1%로 감소 (100%)~~
- ~~원한 서린 코인의 방어력 증가 효과가 20에서 10으로 감소 (100%)~~
- ~~크리스털 이어링의 공격력 증가 효과가 15에서 10으로 감소 (100%)~~
- ~~데스티니 로브 방어력 효과가 62에서 60으로 감소 (100%)~~
- ~~낡은 코인의 크리티컬 증가 효과가 180%에서 200%으로 상향 (100%)~~
- ~~누군가의 반지의 반격 보정 효과와 크리티컬 확률 증가 효과가 1% 상향되며, 크리티컬 확률 강화가 매 강화마다 1%씩 증가하도록 상향 (100%)~~
- ~~퍼루브타 테네브리스의 공격력이 80에서 84로 상향 (100%)~~
- ~~라스 웜의 가죽 장갑의 방어력 효과가 68에서 70으로 상향 (100%)~~
- ~~6 챕터 레어 아이템의 등장 확률이 2% 증가하며, 원더 아이템의 등장 확률이 5% 감소 (100%)~~
- ~~레드 클로크의 이미지와 이름이 퍼플 클로크로 변경이 되며, 서늘한 원혼에게서 나오도록 변경한다. (100%)~~

### 11) 12주 간단한 아이템 패치
**추가 사항**
- ~~상점에서 해금할 수 있는 아이템을 네 가지 추가한다. (100%)~~
- ~~상점 해금 아이템인 푸른 마법책을 추가한다. 푸른 마법책은 티스 웜에게서 나오는 크리티컬 확률 4%, 공격력 4를 증가시키는 D등급 Rare 부적 아이템이다. (100%)~~
- ~~상점 해금 아이템인 캄 플라워를 추가한다. 캄 플라워는 그로겔에게서 나오는 크리티컬 확률 8%, 반격 보정 확률 2%를 증가시키는 B등급 Normal 부적 아이템이다. (100%)~~
- ~~상점 해금 아이템인 어썰트 대거를 추가한다. 어썰트 대거는 블루 울프에게서 나오는 공격력 5, 반격 보정 확률 2%를 증가시키는 E등급 Rare 단검 무기 아이템이다. (100%)~~
- ~~상점 해금 아이템인 페인 쉴드를 추가한다. 페인 실드는 불안정한 원혼에게서 나오는 공격력 38, 방어력 11를 증가시키는 R등급 Wonder 방패 무기 아이템이다. (100%)~~
- ~~Act 1을 클리어하면 자동적으로 해금되는 아이템인 나이트 글로브를 추가한다. 나이트 글로브는 그린 울프에게서 나오는 방어력 16, 공격력 3를 증가시키는 D등급 Wonder 방어구 아이템이다. (100%)~~
- ~~Act 2을 클리어하면 자동적으로 해금되는 아이템인 섀도우 넥클레스를 추가한다. 섀도우 넥클레스는 블루 스콜피온에게서 나오는 반격 보정 확률 18%를 증가시키는 B등급 Wonder 장신구 아이템이다. (100%)~~
- ~~새로운 특성인 치환을 추가한다. 치환은 상점에서 해금할 수 있는 특성으로, 반격을 받으면 그 전투 동안 받을 때마다 크리티컬 데미지가 25% 증가하는 효과가 있다. (100%)~~
- ~~탐색과 치환이 강화가 가능하도록 변경한다. (100%)~~
- ~~SR 아이템, SSR 아이템이 강화가 가능하도록 변경한다. (100%)~~

**변경 사항**

- ~~스테이지 표시가 이제 해당 챕터의 진행도 표시로 바뀌며, 왼쪽에 Stage 글 대신에 해당 챕터의 장소를 표시한다. (100%)~~
- ~~투지의 방패 공격력 47에서 50으로 상향 (100%)~~
- ~~투지의 방패의 공격력 강화가 매번 5씩 오르도록 상향 (100%)~~

### 11) 13주 개선 패치 시작
**추가 사항**
- ~~로비와 특성 화면에 튜토리얼이 추가한다. (100%)~~
- ~~아이템 정보에 해당 아이템이 어떤 조합 아이템의 재료인지 표기하도록 내용을 추가한다. ( 자리는 아이템의 능력과 설명 사이 ) (100%)~~
- ~~Rare 이상 희귀성 아이템의 프레임에 해당 희귀성의 색깔에 맞는 외곽선을 추가한다. (100%)~~
- ~~대사 스킵, 이전으로 돌아가기 버튼을 추가한다. (100%)~~
- ~~고대석과 복구석이 강화가 되도록 만든다. ( 고대석은 수리 수치 + 1, 복구석은 + 10% ) (100%)~~

**변경 사항**

- ~~상점과 사전에 대화 혹은 보상이 있는 것을 아이콘만이 아닌 텍스트로 표기되게 변경한다. (100%)~~
- ~~장착 아이템의 Delete키가 전투 중에도 보이도록 변경한다. (100%)~~
- ~~상기 상자에 나오는 아이템이 최대 세 개로 변경한다. (100%)~~
- ~~게임 종료 시에 받는 강몽석의 점수 조건이 변경한다. ( 60,000 -> 55,000 ) (100%)~~
- ~~스토리, 마녀 스토리, 보스 몬스터와의 대사에서 글자가 잘려서 줄바꿈 되는 것을 보정한다. (100%)~~
- ~~등급, 희귀성 필터일 때 아이템이 나오지 않던 버그를 수정한다. (100%)~~
- ~~아이템 정보의 Delete를 통해서 버그가 일어나던 버그를 수정한다. (100%)~~

### 12) 14주 UI 개선 패치
**추가 사항**
~~- 자동 장착 시스템을 추가한다. (100%)~~
~~- 자동 장착 시스템이 활성화된 상태에서 장비가 파괴되면 그 장비 아이템 이하의 내구도 수치를 가진 아이템을 자동으로 장착한다. (100%)~~
~~- 만약에 내구도 수치 이하의 아이템이 없을 경우, 내구도 수치를 초과하는 아이템을 자동으로 장착한다. (100%)~~
~~- 아이템을 잠그는 시스템을 추가한다. 잠겨진 아이템은 붉게 변하며, 사용 및 삭제가 불가능해진다. (100%)~~

**변경 사항**

~~- 모든 UI로 도트로 변경한다. ( 단, 아이템은 아직 되지 않았으니 이후에 한다. ) (100%)~~
~~- 상점의 아이템 정보 UI를 구입 팝업창과 통합시킨다. (100%)~~
~~- 아이템의 마나 필요 표시는 사용 버튼의 텍스트가 변경되는 방식으로 변경한다. (100%)~~
~~- 몬스터 등장 연출 버튼을 삭제한다. (100%)~~
~~- 여정의 옵션에 있는 자동 장착 버튼, 인벤-가방 아이템 보상 수령 방식 버튼, 아이템 수령 경고 버튼을 바깥으로 옮긴다. (100%)~~
~~- 어둠 속에 존재하는 자와의 패배로 재몽석을 사용할 경우, 포기하기를 누르면 게임에서 패배하는 버그를 수정한다. (100%)~~
~~- 아이템 설명 부분 오타가 있는 것을 수정한다. (100%)~~
~~- PC에서 Space 버튼으로 공격 및 진행하기 버튼이 눌려지도록 변경한다. (100%)~~
~~- 스킵한 이후, 대화가 다시 출력되면 스킵 팝업창이 켜지던 버그를 수정한다. (100%)~~
~~- 치환 특성이 상점에서 충전이라는 이름으로 되어 있던 버그 수정한다. (100%)~~

### 13) 15주 조우 요소 추가 패치
**추가 사항**
- ~~특수 상자를 추가한다. 특수 상자에서는 Wonder 아이템 1~2개, Rare 아이템 3~4개, 총합 4~5개의 아이템이 무조건 들어 있다. (100%)~~
- ~~특수 상자에서 아이템들 중에서 하나의 아이템을 무작위로 얻을 수 있다. 이 상자의 출현확률은 1/2%다. (100%)~~
- ~~상점에 튜토리얼을 추가한다. (100%)~~

**변경 사항**

- ~~내구도 감소량, 점수에 ':' 표시를 추가한다. (100%)~~
- ~~Chapter와 Act에 표시되었던 '.'를 삭제한다. (100%)~~
- ~~조합법 필터 버튼이 작동 안 하던 버그를 수정한다. (100%)~~
- ~~조합법의 각 아이템의 박스 우측이 조금 잘리던 현상을 수정한다. (100%)~~
- ~~사전, 상점 필터 활성화 시에 배경이 추가되도록 변경한다. (100%)~~
- ~~로비 중앙 버튼, 가방 아이템의 단축키, 아이템 필터 체크박스, 장착 아이템 이벤토리 UI를 변경시킨다. (100%)~~

### 14) 16주 새로운 모드 추가
**추가 사항**
- ~~새로운 모드인 Slow, Very Slow, Trial을 추가한다. 이 모드들의 특징은 각각 한 챕터의 스테이지를 25/30/40개로 늘린다. (100%)~~
- ~~Noraml, Fast, Very Fast와 별개로 또 Slow, Very Slow, Trial 랭킹을 추가한다. (100%)~~
- ~~아이템 정보 확대하기 버튼을 추가한다. 해당 버튼은 아이템 잠금 좌측에 위치한다. (100%)~~
- ~~아이템 정보 확대하기 버튼을 누르면, 해당 아이템의 정보가 화면의 중앙에 더 큰 화면으로 출력된다. 단, 버튼들은 추가되지 않는다. (100%)~~
- ~~새로운 조합 아이템 샤이닝 그레이트 소드를 추가한다. 샤이닝 그레이트 소드는 SR등급 Wonder 검 무기 아이템으로, 공격력을 104 증가해준다. ( 조합법은 대검 + 은 검 + 샤이트 라이플 + 라이트 라이플 ) (100%)~~
- ~~스테이지 현황 UI를 클릭하면 전체 스테이지 혹은 해당 챕터의 스테이지 진행 현황이 뜨도록 시스템을 추가한다. (100%)~~

**변경 사항**

- ~~일부 UI가 깨지지 않도록 크기와 위치를 조정한다. (100%)~~
- ~~옵션 팝업창에서 다른 쪽을 클릭하면 꺼지도록 변경한다. (100%)~~
- ~~거목의 뿌리의 강화에 필요한 강화석 비용이 2 더 적었던 버그를 수정한다. (100%)~~
- ~~조합 아이템 정보 중에 만약에 얻지 않은 아이템일 경우, 해당 아이템을 재료로 하는 아이템이 없다는 문구가 출력되지 않던 버그 수정한다. (100%)~~
- ~~일부 대화 중 너무 긴 것은 대사를 두 개로 나눈다. (100%)~~
- ~~메인 화면의 버튼을 모두 한글로 바꾼다. (100%)~~

### 15) 17주 버그 수정
**추가 사항**
- ~~접몽 석상의 조합에 이미 넣은 재료를 뺄 수 있는 빼내기 버튼을 추가한다. (100%)~~
- ~~상점 해금 아이템인 리볼버를 추가한다. 리볼버는 거대 들쥐에게서 나오는 공격력 5 증가, 크리티컬 데미지 10% 증가 효과를 가진 E등급 Normal 총 무기 아이템이다. (100%)~~
- ~~도감에 미획득 혹은 미처치 요소들만 출력되는 필터를 추가한다. (100%)~~

**변경 사항**

- ~~스테이지의 챕터와 숫자가 겹치는 현상을 수정한다. (100%)~~
- ~~중독 상태이상 글자가 누락되는 버그를 수정한다. (100%)~~
- ~~보상 아이템의 배경 크기 버그를 수정한다. (100%)~~
- ~~가방 아이템에서 외곽선 달린 칸이 스크롤이 넘어가도 표시되는 버그를 수정한다. (100%)~~
- ~~가방 아이템의 스크롤 높이가 비정상이던 버그를 수정한다. (100%)~~
- ~~Tip 중에 {Emter}라고 뜨는 팁의 오타를 수정한다. (100%)~~
- ~~모바일에서 가방의 단축키 옵션이 표시되던 버그를 수정한다. (100%)~~
- ~~옵션 창이 음량을 조절하면 자동으로 꺼지던 버그를 수정한다. (100%)~~
- ~~인게임의 옵션 창 음량 조절이 저장되지 않던 버그를 수정한다. (100%)~~
- ~~특수 상자의 아이템 얻기 소리가 BGM 음량에 따르던 버그를 수정한다. (100%)~~
- ~~접몽 석상 창의 석상의 크기가 이상하던 버그를 수정한다. (100%)~~
- ~~랭킹에서 슬로우 ~ 트라이얼 모드의 Act 표기가 이상하던 버그를 수정한다. (100%)~~
- ~~랭킹에서 슬로우 ~ 트라이얼 모드에서 뉴 스코어 표기가 수시로 뜨던 버그를 수정한다. (100%)~~
- ~~Act 2의 짙은 배경의 효과 크기가 맞지 않던 버그를 수정한다. (100%)~~
- ~~대화 종료 이후에 이벤트가 있는 대화를 종료하면 이벤트가 발생하지 않던 버그를 수정한다. (100%)~~
- ~~접몽 석상, 상기 상자, 특수 상자를 열 시에 점수를 주도록 변경된다. (100%)~~
- ~~구입 소리의 음량을 조금 줄인다. (100%)~~
- ~~외곽선의 크기를 조정한다. (100%)~~
- ~~접몽 석상과 조합법에 표기된 재료 아이템들에 누르면 정보가 출력되는 시스템을 추가한다. (100%)~~
- ~~상기 상자와 특수 상자에 가방 아이템을 추가한다. (100%)~~
- ~~몽환석을 점수 / 90만큼 얻게 변경한다. (100%)~~
- ~~육중의 강화 수치가 60에서 45로 하향한다. (100%)~~

### 16) 18주 튜토리얼 작업 마무리
**추가 사항**
- ~~인게임 튜토리얼을 추가한다. 인게임 튜토리얼은 현재 화면의 UI를 모두 설명하는 튜토리얼이다. (100%)~~
- ~~특정 상황에서 튜토리얼이 출력되도록 한다. 이 상황의 튜토리얼에서는 한 번에 모든 것을 알려주도록 만든다. (100%)~~
- ~~상점 해금 아이템인 불안의 조각을 추가한다. 불안의 조각은 포이즌 웜에게서 나오는 크리티컬 확률 12% 증가, 공격력 8 증가 효과를 가진 B등급 Wonder 부적 아이템이다. (100%)~~
- ~~조합 아이템인 피프스 애뮬렛을 추가한다. 피프스 애뮬렛은 R등급 Unique 장신구 아이템으로, 반격 보정 확률을 28%, 공격력을 8, 방어력을 8 증가시켜준다. ( 조합법은 목걸이 + 라피스 라줄리 목걸이 + 엘프 넥클레스 + 투지의 보석 아니모 ) (100%)~~
- ~~상점을 언급하는 대화를 추가한다. (100%)~~

**변경 사항**

- ~~일부 튜토리얼 대화를 삭제한다. (100%)~~
- ~~튜토리얼을 여정의 시작으로 표기하도록 변경한다. (100%)~~
- ~~네 가지 종류의 장비 아이템 튜토리얼 대화를 하나로 묶는다. (100%)~~
- ~~단검, 푸른 마법책, 메탈 크로스보우, 베네디캇이 상자에서 나오도록 변경한다. (100%)~~

### 17) 19주 인물 사전 작업
**추가 사항**
- ~~인물 사전을 추가한다. (100%)~~
- ~~인물 사전의 인물들은 조건에 따라서 열리게 만들며, 잠겨진 인물들은 아이템이나 몬스터처럼 모두 비공개로 만든다. (100%)~~
- ~~잠겨진 인물은 이미지가 검게 출력되며, 비공개된 텍스트는 모두 ???로 만든다. (100%)~~
- ~~인물의 스토리 설명은 분기마다 구분선을 둔다. (100%)~~
- ~~인물의 내용에는 이름, 이미지, 대표적인 말, 능력, 성격, 좋아하는 것, 싫어하는 것, 대표 설명, 스토리 설명이 있다. (100%)~~
- ~~모든 필터창, 팝업창, 설정창을 바깥을 누르면 Cancel 버튼 시스템이 작동되도록 만든다. (100%)~~
- ~~접몽 석상에서 조합을 진행하지 않은 상태일 경우, 돌아가는 버튼이 켜지도록 만든다. 또한, 돌아가기 버튼을 누르면 다시 상자를 선택할 수 있는 곳으로 돌아오도록 만든다. (100%)~~

**변경 사항**

- ~~몬스터들의 강몽석 드랍 확률을 2%로 증가시킨다. (100%)~~
- ~~어둠 속에 존재하는 자의 강몽석 드랍 확률을 100%로 변경한다. (100%)~~
- ~~의문의 상자의 몽환석과 강몽석의 지급량을 1.5 ~ 2배만큼 증가시킨다. (100%)~~
- ~~점수 비례 몽환석 지급량을 점수 / 90에서 점수 / 55로 낮춘다. (100%)~~
- ~~점수 비례 강몽석 지급량을 점수 / 55,000에서 점수 / 35,000로 낮춘다. (100%)~~
- ~~사전의 아이콘이 눌러진 상태면 밝은 상태로 변경되도록 수정한다. (100%)~~
- ~~대사에 생각과 대화 표현 구분한다. (100%)~~
- ~~너클류 무기 아이템의 이름 변경한다. (100%)~~
- ~~이름 입력칸을 삭제한다. (100%)~~
- ~~접몽 석상 선택 시 지급되던 점수를 더 이상 주지 않게 만든다. (100%)~~
- ~~보스 아이템 조우 대화가 Act 1에서만 보여지도록 변경한다. (100%)~~
- ~~사전의 맨 오른쪽에 있는 아이템의 이름이 잘리는 버그 수정한다. (100%)~~

### 18) 20주 가방 개선 작업
**추가 사항**
- ~~가방을 얻을 경우, 옵션 기능이 켜져 있으면 해당 가방 아이템에 대한 선택지 팝업창이 나오도록 변경한다. (100%)~~
- ~~가방 선택지 팝업창에서 얻을 가방 아이템보다 수용량이 적은 가방 아이템을 누르면 내용물 옮기기, 가방 바꾸기 버튼이 활성화된다. (100%)~~
- ~~가방 선택지 팝업창에서 내용물만 옮기기 버튼을 누르면 해당 선택 가방 아이템의 내용물들이 얻을 가방 아이템을 얻고, 그 안에 모두 이동된다. (100%)~~
- ~~가방 선택지 팝업창에서 가방 바꾸기 버튼을 누르면, 선택한 가방 아이템을 내용물만 추출하고 삭제한 뒤, 얻을 아이템을 얻고 그 안에 모두 이전 가방의 내용물들을 채워넣는다. (100%)~~
- ~~가방 선택지 팝업창에서 가방이 8개가 있으면 그냥 얻기 버튼을 받지 않기 버튼으로 바꾼다. (100%)~~
- ~~가방 선택지 팝업창에서 가방이 7개 이하 있으면 받지 않기 버튼을 그냥 얻기 버튼으로 바꾼다. (100%)~~
- ~~새로운 특성인 증력을 추가한다. 증력 특성은 최대 마력을 3 올려주는 기본 특성이다. (100%)~~
- ~~메인 홈, 게임 설정, 특성, 상점 튜토리얼을 모두 보면 홈 화면에서 보상을 주도록 만든다. ( 각각 보상은 몽환석 5,000, 강몽석 50, 몽환석 5,000, 재몽석 1이다. ) (100%)~~

**변경 사항**

- ~~홈 화면의 버튼들에 마우스를 올리거나 누르면 하얀 하이라이트가 출력되도록 변경한다. (100%)~~
- ~~인게임 옵션에 검은 뒷배경이 출력되지 않던 버그를 수정한다. (100%)~~
- ~~아이템 구입창의 아이템 종류 텍스트 출력 버그를 수정한다. (100%)~~
- ~~탐색 특성의 유물이라는 단어를 상자로 롤백한다. (100%)~~
- ~~로그의 길이를 화면의 높이와 똑같게 만든다. (100%)~~
- ~~일부 아이템 설명을 변경하며 오타를 수정한다. (100%)~~
- ~~탐색, 제약, 마강 특성을 상점에서 해금해야 할 수 있는 특성으로 변경한다. (100%)~~
- ~~특성 가격을 3,000으로 변경한다. (100%)~~
- ~~마강의 최대 마나 버그를 수정한다. (100%)~~
- ~~엔딩의 몽환석 지급량이 낮게 표시되던 버그를 수정한다. (100%)~~
- ~~UI 위치를 조정한다. (100%)~~

### 19) 21주 지식 사전 추가
**추가 사항**
- ~~지식 사전을 추가한다. (100%)~~
- ~~지식 사전의 인물들은 조건에 따라서 열리게 만들며, 잠겨진 인물들은 아이템이나 몬스터처럼 모두 비공개로 만든다. (100%)~~
- ~~잠겨진 지식은 이미지가 검게 출력되며, 비공개된 텍스트는 모두 ???로 만든다.  (100%)~~
- ~~지식의 스토리 설명은 분기마다 구분선을 둔다. (100%)~~
- ~~지식의 내용에는 이름, 대표 이미지, 대표 설명, 스토리 설명이 있다. (100%)~~
- ~~능력치 소비 아이템을 자동으로 사용하도록 변경한다. 능력치 소비 아이템이 인벤토리에 추가되면 자동적으로 사용되도록 시스템을 만든다. (100%)~~
- ~~능력치 소비 아이템 자동 사용 시스템은 옵션에서 On/Off로 끄거나 킬 수 있도록 변경한다. (100%)~~

**변경 사항**

- ~~B등급 이상의 고대석, 능력치 소비 아이템을 제외한 모든 아이템들의 최대 강화를 5로 증가한다. (100%)~~
- ~~보상의 팝업창이 바깥의 검은 부분을 누르면 꺼지도록 수정한다. (100%)~~

### 20) 22~23주 영문화 및 언어 변경 추가
**추가 사항**
- ~~모든 스트링과 UI 텍스트들의 영문화를 준비한다. (100%)~~
- ~~언어를 설정하지 않은 상태이면, 게임을 시작할 때 언어를 설정하도록 변경한다. (100%)~~
- ~~체력 회복 아이템을 자동으로 사용하는 시스템을 추가한다. 또한, 이 시스템은 인게임의 옵션에서 On/Off 기능으로 끄거나 킬 수 있다. (100%)~~
- ~~체력 회복 아이템 자동 사용 시스템은 몬스터에게 데미지를 받을 경우, 현재 체력이 인벤토리에 있는 체력 아이템으로 회복될 경우의 수치가 최대 체력을 넘지 않을 경우에 계속해서 아이템을 자동으로 사용한다. (100%)~~

**변경 사항**

- ~~홈 화면 옵션에 언어를 설정할 수 있는 버튼을 추가한다. (100%)~~
- ~~보상을 모두 받았을 경우, 혹은 상기 상자의 아이템을 모두 구입한 경우 보상 창이 자동으로 꺼지도록 변경한다. (100%)~~
- ~~튜토리얼의 화면 필터가 더 어두워지도록 변경한다. (100%)~~

### 21) 24~25주 스프라이트 변경 및 시놉시스 추가
**추가 사항**
- ~~모든 몬스터 스프라이트, 챕터 배경 스프라이트 ( 어둠 속에 존재하는 자 전투 배경 포함 )을 변경한다. (100%)~~
- ~~Act 2에서 등장하는 모든 몬스터들의 밝기를 조금 줄이고, 희미한 검은 연기가 피어오르는 연출을 추가한다. (100%)~~
- ~~몬스터의 애니메이션이 변경됨에 따라서 몬스터 사망 연출을 통제하는 코드를 수정한다. (100%)~~

**변경 사항**

- ~~Act에 소제목을 추가한다. (100%)~~
- ~~첫 번째 로딩에서는 시놉시스가 나오도록 변경한다. (100%)~~
- ~~마녀 대화의 대화 제목이 안 나오던 버그를 수정한다. (100%)~~
- ~~영문에서 마녀 대화가 읽음 처리로 되지 않던 버그를 수정한다. (100%)~~
- ~~조합의 빼기 버튼이 영문화가 안 되던 버그를 수정한다. (100%)~~
- ~~빼기 버튼을 사용하면 조합 닫기 버튼이 고장나는 버그를 수정한다. (100%)~~
- ~~아이템, 특성 강화 시의 표기 버그를 수정한다. (100%)~~
- ~~가방 받기 버그를 수정한다. (100%)~~
- ~~기록소 지식 쪽의 용사가 있는 곳이 영어명 때에는 이름 칸을 뛰어넘던 버그를 수정한다. (100%)~~
- ~~특성 튜토리얼이 출력되지 않던 버그를 수정한다. (100%)~~
- ~~영문에서 용사는 Hero에서 Warrior로, 전사는 Warrior에서 Striker로 변경한다. (100%)~~

### 22) 26주 보스 기억 아이템 추가
**추가 사항**
- ~~보스 몬스터에게서 기억 아이템을 드랍하게 한다. ( Act별로 다른 아이템이 나오도록 만든다. ) (100%)~~
- ~~보스 몬스터의 기억 아이템을 얻으면, 해당 아이템은 인벤토리에 들어가지 않고 삭제된다. 해당 아이템을 얻으면 기록소에서 해당 아이템에 맞는 기억이 해금된다. (100%)~~
- ~~해금된 보스 기억은 소설의 형태로 볼 수가 있다. (100%)~~
- ~~보스 기억을 해금하면 보상으로 몽환석 200을 받을 수가 있다. (100%)~~

**변경 사항**

- ~~전투 확률 바를 복구시킨다. (100%)~~
- ~~홈 화면, 상점 화면, 치유의 숲 배경, 이상한 공간의 배경을 모두 바꾼다. (100%)~~
- ~~가방 버그를 수정한다. (100%)~~
- ~~Act 2 어둠 속에 존재하는 자의 전투 패배 이후 오라가 사라지지 않던 버그를 수정한다. (100%)~~

### 23) 27주 UI 변경
**추가 사항**
- ~~3가지 퀘스트를 추가한다. 퀘스트에는 스토리 퀘스트, 반복 퀘스트, 목표 퀘스트가 있다. (100%)~~

**변경 사항**

- ~~대부분 UI를 모두 개선된 디자인으로 준비된 UI 스프라이트들로 교체한다. (100%)~~
- ~~단축키로 생성된 가방 창은 마우스 아래가 아닌 모바일과 동일한 위치에서 생성되도록 수정한다. (100%)~~
- ~~Alt + Enter를 방지하도록 유니티에서 설정한다. (100%)~~
- ~~Act 1 엔딩 이후 한글 마녀 대사의 오타를 수정한다. (100%)~~
- ~~영어 아이템 데이터가 잘 불러오지 못했던 버그를 수정한다. (100%)~~
- ~~튜토리얼, 대화 넘기기, 보상 완료에 스페이스 바 단축키를 추가한다. (100%)~~
- ~~내구도 튜토리얼의 박스 위치가 이상하던 버그를 수정한다. (100%)~~

### 24) 28주 퀘스트 추가
**추가 사항**
- ~~3가지 퀘스트를 추가한다. 퀘스트에는 스토리 퀘스트, 반복 퀘스트, 목표 퀘스트가 있다. (100%)~~
- ~~스토리 퀘스트는 Act 1 여정 모두 클리어, Act 1 클리어 이후 마녀 대화, Act 2 진입 이전 마녀 대화, Act 2 여정 클리어, Act 2 여정 클리어 이후 마녀 대화, 베타 및 데모 안내를 한다. (100%)~~
- ~~스토리 퀘스트( 스토리 길잡이 ) UI는 홈 화면의 우측에 위치해 있다. (100%)~~
- ~~반복 퀘스트를 추가한다. 반복 퀘스트는 클리어할 때마다 카운터가 하나 쌓이고, 카운터는 해당 퀘스트 이름의 옆에 가로와 함께 표시된다. (100%)~~
- ~~반복 퀘스트에는 마물 처치, 보스 처치, 히든 보스 처치, 여정 클리어, 치유의 숲 조우, 이상한 공간 조우, 아이템 조합, 마물에게서 Unique 아이템 / Epic 아이템 / Legend 아이템 얻기 퀘스트가 있다. (100%)~~
- ~~퀘스트 UI 안에는 퀘스트의 이름, 퀘스트의 내용, 퀘스트의 진행 슬라이더 바와 진행도(숫자), 보상, 보상 받기 버튼이 있다. (100%)~~
- ~~목표 퀘스트를 추가한다. 목표 퀘스트는 해당 목표를 달성하면 클리어되는 방식이며 반복 퀘스트처럼 카운터가 쌓인다. (100%)~~
- ~~목표 퀘스트에는 아이템 사전 채우기, 마물 사전 채우기, 보스 기억 아이템 모으기, 아이템이나 특성 해금하기, 아이템이나 특성 강화하기가 있다. (100%)~~
- ~~더 이상의 목표 퀘스트를 진행할 수 없을 경우, 퀘스트의 맨 위에 체크 표시 아이콘을 띄운다. (100%)~~
- ~~퀘스트의 출력 순은 보상을 받을 수 있는 퀘스트 > 보상을 못 받는 퀘스트 > 완료된 퀘스트 순이다. (100%)~~

**변경 사항**

- ~~일부 UI를 개선된 스프라이트로 변경한다. (100%)~~
- ~~모든 폰트를 변경한다. (100%)~~

### 25) 29주 퀘스트 및 아이콘 작업 완료
**추가 사항**
- ~~일일 퀘스트를 추가한다. 일일 퀘스트는 다음 날 오전 9시 이후면 갱신되도록 만들며, 인터넷이 연결되어 있을 때 갱신되도록 설정한다. (100%)~~
- ~~특수 퀘스트를 추가한다. 특수 퀘스트는 정말 달성하기 어려운 임무인 퀘스트만이 출력된다. (100%)~~
- ~~일일 퀘스트는 갱신이 되면 10개의 퀘스트 중 랜덤으로 네 개의 퀘스트를 뽑아 갱신하며, 일일 퀘스트를 클리어하는 퀘스트는 고정적으로 추가한다. (100%)~~
- ~~일일 퀘스트의 목록에는 일일 퀘스트 클리어, 몬스터 처치, 보스 처치, 아이템 얻기, 무기나 방어구 아이템 얻기, 장신구나 부적 아이템 얻기, 상자 발견하기, 체력 아이템 사용하기, 가방 아이템 얻기, 마법 아이템 얻기, 장비 아이템 파괴되기가 있다. (100%)~~
- ~~특수 퀘스트의 목록에는 해당 보스 몬스터의 아이템을 착용한 상태로 그 보스를 쓰러뜨리기가 각 보스마다 있다. (100%)~~
- ~~조합 아이템 중에서 재료가 이전 챕터까지만 나오는 재료가 있으면 아이템 정보나 조합 목록에서 나오는 재료를 붉게 표시하며, 조합 아이템 목록 상단에 재료가 더 이상 안 나오는 아이템이라고 표기한다. (100%)~~

**변경 사항**

- ~~모든 아이콘을 새로 만들어진 도트 아이콘으로 변경한다. (100%)~~
- ~~상태이상이나 버프의 잔여 턴을 중앙에서 우측 하단에 표기하도록 설정한다. (100%)~~
- ~~아이템 조합법에서 보이는 관심 버튼을 누르면 배경이 보이도록 변경한다. (100%)~~
- ~~반복 퀘스트의 보상을 25% 증가한다. (100%)~~
- ~~일부 UI의 디자인을 변경한다. (100%)~~
- ~~일부 UI의 폰트 색을 변경한다. (100%)~~

### 26) 30주 전시회 의견 반영 1차
**추가 사항**
- ~~사용 및 파괴, 모두 얻기에 단축키 추가 ( 각각 A,S,Q ) (100%)~~
- ~~해금되지 않은 아이템은 조합법에서 얻지 못하는 재료가 있는 아이템이라고 안내 (100%)~~

**변경 사항**

- ~~실렌티움 공격력 60 -> 55, 공격력 강화율 6 -> 5 (100%)~~
- ~~포이즌 이그니스 공격력 50 -> 60, 공격력 강화율 5 -> 6 (100%)~~
- ~~골궁 공격력 40 -> 50, 공격력 강화율 4 -> 5 (100%)~~
- ~~오래 전 용사의 견갑 조합법 변경 (100%)~~
- ~~퍼루브타 테네브리스 조합법 변경 (100%)~~
- ~~글라디우스 A Wonder -> A Unique / 공격력 58 -> 공격력 63 / 강몽석 비용 8 -> 9 (100%)~~
- ~~누군가의 반지 R Wonder -> R Unique / 반격 보정 25% -> 28% / 크리티컬 확률 8% -> 10% / 반격 보정 확률 강화율 2% -> 3% 강몽석 비용 12 -> 13 (100%)~~
- ~~괴기를 품은 두개골 S Wonder -> R Unique / 크리티컬 확률 26% -> 23%, 크리티컬 데미지 50% -> 80% (100%)~~
- ~~투지의 방패 R Unique -> S Wonder / 공격력 50 -> 52, 방어력 15 -> 20 / 방어력 강화율 1 -> 2 (100%)~~
- ~~오래 전 영웅의 견갑 S Unique -> SR Wonder (100%)~~
- ~~거목의 뿌리 S Wonder -> R Unique / 공격력 75 -> 60, 크리티컬 확률 12% -> 10% / 공격력 강화율 7 -> 6 (100%)~~
- ~~예리한 울프 스워드 공격력 75 -> 79 / 공격력 강화율 7 -> 8 (100%)~~
- ~~오래 전 대장장이의 주괴 R Unique -> S Unique / 크리티컬 데미지 165% -> 260%, 크리티컬 확률 15% -> 12% / 크뎀 강화율 21% -> 32%  (100%)~~
- ~~피프스 애뮬렛 R Unique -> S Unique / 반격 보정 28% -> 32%, 강몽석 비용 13 -> 15 (100%)~~
- ~~그로겔 이어링 반격 보정 32% -> 28% (100%)~~
- ~~원혼이 담긴 반지 반격 보정 35% -> 30% (100%)~~
- ~~검은 반지 반격 보정 40% -> 43% (100%)~~
- ~~투지의 보석 아니모 반격 보정 18% -> 20%, 크리티컬 데미지 45% -> 55% / 크리티컬 데미지 강화율 5% -> 7% (100%)~~
- ~~크레센트 링 반격 보정 30% -> 28% (100%)~~
- ~~두스 반격 보정 33% -> 30% (100%)~~
- ~~포레스트 링 반격 보정 32% -> 28% (100%)~~
- ~~루비 반지 반격 보정 22% -> 21% (100%)~~
- ~~보상 모두 받기 단축키가 생깁니다. ( Q로 조작이 가능합니다. ) (100%)~~
- ~~완료 버튼 단축키가 Space에서 W로 변경 (100%)~~
- ~~얻지 못하는 아이템은 자동으로 관심에서 빠지도록 수정 (100%)~~
- ~~전투 UI 텍스트가 10줄이 넘으면 맨 위의 텍스트가 삭제되도록 변경 (100%)~~
- ~~소형 포션이 2챕터 이상에서는 얻을 수 없는 아이템이라고 뜨는 버그 수정 (100%)~~
- ~~마녀의 이미지가 변경 (100%)~~
- ~~가방이 화면 중앙에서 나오도록 변경 (100%)~~
- ~~가방 아이템 착용 버그 수정 (100%)~~
- ~~해상도 개선 1차 작업 (100%)~~
- ~~조합법에 재료 아이템이 어디서 나오는지 표기 (100%)~~
- ~~잠긴 장비 아이템이 드래그하면 장착되는 버그 수정 (100%)~~
- ~~괴기를 품은 두개골 강화 수치가 이상하던 버그 수정 (100%)~~
- ~~일부 UI 디자인 변경 (100%)~~

### 27) 31주 전시회 의견 반영 2차
**추가 사항**
- ~~조합할 때 재료를 모두 자동으로 넣는 버튼을 추가한다. (100%)~~
- ~~자동 넣기 버튼을 누르면 내구도가 가장 많이 닳은 아이템이 인벤토리에서 우선적으로 재료로 들어가게 된다. (100%)~~
- ~~보상을 수령하는 창에서 가방, 마법, 기타 아이템을 제외한 아이템들을 바로 사용할 수 있는 버튼을 추가한다. (100%)~~
- ~~보상에서 장비 아이템을 바로 장착할 경우, 해당 종류의 장비가 비어 있는 상태면 바로 장착된다. (100%)~~
- ~~보상에서 장비 아이템을 바로 장착할 경우, 해당 종류의 장비가 장착된 상태일 경우면 인벤토리가 하나 이상 빈 상태에서 장비를 교체하게 바꾼다. (100%)~~

**변경 사항**

- ~~일일 퀘스트와 반복 퀘스트 보상이 일부 증가한다. ( 각각 40 -> 50, 20% 증가 ) (100%)~~
- ~~기록소에서 보상 수령 버튼이 해당 창에서 받을 수 있는 보상이 있을 경우 출력되게 변경된다. (100%)~~
- ~~가방을 닫을 때 드래그된 아이템이 있으면 초기화되게 변경한다. (100%)~~
- ~~인게임의 옵션 창과 아이템을 얻을 수 없는 창들은 ESC로 끌 수 있도록 변경된다. (100%)~~
- ~~모바일에서 단축키가 텍스트로 나오는 버그를 수정한다. (100%)~~
- ~~첫발이 이제 슬로우, 베리 슬로우, 트라이얼에서는 각각 4,5,6 스테이지까지 상자 조우 확률을 올리도록 변경한다. (100%)~~

### 28) 32주 전시회 의견 반영 3차
**추가 사항**
- ~~비어있는 장비 칸에 빠르게 장비를 장착할 수 있는 키를 추가하며, 비어있는 장비 칸을 누르면 빠르게 장비를 장착할 수 있게 한다. (100%)~~
- ~~빠르게 장비를 장착할 경우에는 등급이 가장 낮고, 내구도가 가장 낮은 무기를 장착한다. (100%)~~
- ~~빠르게 장비를 장착하는 단축키는 무기, 방어구, 장신구, 부적 각각 Q,W,E,R로 설정한다. (100%)~~
- ~~능력치를 보는 UI는 버튼이나 C키를 눌러서 키거나 끌 수 있도록 변경하며, 위치는 기존 위치의 바로 좌측으로 옮긴다. (100%)~~
- ~~마법 아이템을 개별로 보관하는 인벤토리 창을 추가한다. 이때, 마법 아이템 인벤토리 칸은 8개이며, 기본 인벤토리 UI처럼 드래그 기능은 포함하지 않는다. (100%)~~
- ~~마법 아이템은 최대 세 개까지 중첩하여 모을 수가 있다. (100%)~~
- ~~마법 아이템을 얻을 때, 인벤토리 안에 보유 수가 2개 이하인 마법이 있으면, 해당 아이템에 중첩한다. 아닐 경우에는 비어있는 인벤토리 칸에 추가한다. 만약에, 마법 아이템이 모두 차면 Max로 표시한다. (100%)~~
- ~~마법 아이템은 기존처럼 사용할 수 있게 한다. (100%)~~
- ~~새로운 조합 아이템 소울 스태프를 추가한다. 소울 스태프는 S등급 Unique 스태프 무기 아이템으로, 공격력 80 증가해주며, 80% 확률로 1턴 동안 3단계 동결 상태이상을 부여한다. ( 조합법은 매직 스태프 + 원한의 해골 + 포이즌 스태프 + 폐허의 씨앗 ) (100%)~~

**변경 사항**

- ~~모두 얻기, 완료 버튼이 Q/W에서 A/S로 변경한다 (100%)~~
- ~~아이템 착용 삭제 버튼이 A/S에서 Z/X로 변경한다. (100%)~~
- ~~가방 최대 보유 가능 개수가 8개에서 7개로 변경한다. (100%)~~
- ~~옵션 창이 뜬 상태에서 스페이스 바를 누르면 작동이 되던 버그를 수정한다. (100%)~~

### 29) 33주 슴슴히 개선
**추가 사항**
- ~~새로운 조합 아이템 블랙 블러드 아머를 추가한다. 블랙 블러드 아머는 S등급 Wonder 방어구 아이템으로, 방어력을 70 증가해준다. ( 조합법은 철 갑옷 + 붉은 보석 + 강철 갑옷 + 블랙 부츠 ) (100%)~~
- ~~새로운 조합 아이템 히어로즈 피스톨을 추가한다. 히어로즈 피스톨은 SR등급 Wonder 총 무기 아이템으로, 공격력 92, 크리티컬 데미지 100% 증가해준다. ( 조합법은 리볼버 + 샤프 슈터 + 수르쿨루스 + 영웅의 동전 ) (100%)~~

**변경 사항**

- ~~인벤토리가 꽉 차거나 가방이 최대치에 도달한다는 팝업창을 삭제하고 안내를 몇 초 뒤에 사라지는 텍스트로 출력되도록 변경한다. (100%)~~
- ~~SSR와 SR 등급의 색깔을 변경한다. (100%)~~
- ~~첫발 특성 설명을 변경한다. (100%)~~
- ~~상기 상자의 아이템들이 이미지와 구입 버튼 사이에 이름까지 출력되도록 변경한다. (100%)~~
- ~~챕터 6의 배경을 변경한다. (100%)~~
- ~~Act 1의 마왕의 이미지를 변경한다. (100%)~~
- ~~마법 아이템이 Z 버튼을 눌러서 작동하지 않던 버그를 수정한다. (100%)~~
- ~~랭킹의 양피지 UI를 오래된 양피지 이미지로 변경한다. (100%)~~
- ~~배경의 뒷배경이 몬스터보다 앞에 나오도록 변경한다. (100%)~~
- ~~보스 때 배경이 붉게 변하는 것이 약하게 변하는 것으로 변경한다. (100%)~~

### 30) 34~35주 Act 3 제작
**추가 사항**
- ~~Act 3의 몬스터들의 밸런스를 조절한다. (100%)~~
- ~~Act 3에 나오는 대화들을 추가한다. (100%)~~
- ~~Act 3에 나오는 새로운 전투 시스템인 게이지와 새로운 조작인 방어하기를 추가한다. (100%)~~
- ~~방어하기를 하면 무조건 적에게 공격을 받지만, 받는 피해가 85%까지 감소하며, 적의 게이지가 두 칸 깎인다. (100%)~~
- ~~공격하기 버튼을 누르면 적의 게이지가 한 칸 쌓인다. (100%)~~
- ~~게이지가 최대치에 도달하면, 게이지를 리셋하고 적이 필살기를 사용한다. (100%)~~
- ~~Act 2이후의 대화를 세 가지 추가한다. (100%)~~
- ~~아이템 사용 시에 이펙트가 나오도록 변경한다. (100%)~~

**변경 사항**

- ~~모든 특성이 이제 최대 1까지 강화가 가능하도록 변경한다. (100%)~~
- ~~반사 버프가 받는 데미지가 없어도 적용되도록 변경한다. (100%)~~
- ~~아이템 장착에 이펙트를 추가하며, 기존의 파괴 이펙트를 새로운 이펙트로 대체한다. (100%)~~
- ~~진행하기 버튼 디자인을 변경한다. (100%)~~
- ~~강몽석 가격을 160으로 낮춘다. (100%)~~
- ~~강몽석 획득 점수 기준을 35,000에서 30,000으로 낮춘다. (100%)~~

### 31) 36~37주 Act 4 제작
**추가 사항**
- ~~Act 4의 몬스터들의 밸런스를 조절한다. (100%)~~
- ~~Act 4에 나오는 대화들을 추가한다. (100%)~~

**변경 사항**

- ~~아이템 강화의 강몽석 필요 가격을 모두 25% 낮춘다. (100%)~~
- ~~강몽석 가격을 150으로 낮춘다. (100%)~~
- ~~강몽석 획득 점수 기준을 30,000에서 20,000으로 낮춘다. (100%)~~
- ~~의문의 상자에서 나오는 강몽석의 갯수를 증가시킨다. (100%)~~

### 32) 38~41주 Act 4, 5 제작
**추가 사항**
- Act 5의 몬스터를 추가한다. (50%)
- ~~Act 5에 나오는 대화들을 추가한다. (100%)~~
- ~~Act 4에 사용되는 챕터 6의 배경화면을 추가한다. (100%)~~
- ~~Act 4의 최종 보스에게 사용되는 스프라이트를 추가한다. (100%)~~
- ~~Act 4의 모든 대화집을 영문화한다. (100%)~~
- ~~아이템 일괄 강화 시스템을 추가한다. 아이템 일괄 강화 시스템은 필터에 걸러진 아이템들은 최소 강화부터 한 단계씩 강화할 수 있는 시스템이다. (100%)~~
- ~~홈 화면에 Act와 관련된 튜토리얼을 추가한다. ( Act 2 해금이 되면 출력이 된다. ) (100%)~~

**변경 사항**

- ~~힘의 견갑의 방어력을 21에서 28로 상향하며, 방어력 강화율을 2에서 3으로 상향한다. (100%)~~
- ~~영어 점수 표기가 한국어로 나오던 버그를 수정한다. (100%)~~
- ~~포레스트 링이 엉뚱한 몬스터에게도 나오던 버그를 수정한다. (100%)~~
- ~~의지의 복구석 / 통찰의 복구석 / 한계의 복구석의 버그를 수정한다. (100%)~~

### 33) 42주 개선 작업
**추가 사항**
- ~~소형 접몽 석상을 추가한다. (100%)~~
- ~~소형 접몽 석상은 재몽석처럼 게임 내에서 사용할 수 있는 저장 아이템으로, 소형 접몽 석상을 사용해서 즉석에서 아이템을 조합할 수 있다. (100%)~~
- ~~소형 접몽 석상의 가격은 750으로 우선 측정하며, 소형 접몽 석상이 없을 경우, 인게임에서 소형 접몽 석상 버튼을 비활성화시킨다. (100%)~~
- ~~소형 접몽 석상은 접몽 석상 이용 시 혹은 전투 상황 혹은 대화 상황에서는 켜지지 않게 만든다. (100%)~~
- ~~대화에 분위기에 맞는 음악을 재생하도록 변경한다. (100%)~~
- Act 5의 몬스터들의 밸런스를 조절한다. (50%)
- ~~Act 5에 나오는 대화들을 추가한다. (100%)~~
- ~~Act 5에서 사용되는 대화들을 모두 영문화한다. (100%)~~
- ~~Act 5에서 나오는 효과들을 만든다. ( 몬스터 오라, 배경의 안개 효과 ) (100%)~~

### 34) 43주 개선 작업
**추가 사항**
- ~~Act 3 중간에 나오는 퀘스트를 추가한다. (100%)~~
- ~~퀘스트를 진행하는 도중에는 홈 화면에 해당 퀘스트를 위한 아이템 개수가 표시된다. (100%)~~
- ~~퀘스트를 클리어하기 위한 아이템이 30개 이상 모였을 경우, 상점에서 대화를 진행하여 강화 한계를 증가시킬 수 있고 캐릭터 기본 능력치를 올릴 수 있도록 만든다. (100%)~~
- ~~퀘스트를 클리어한 이후에는 상점의 아이템 강화는 피르마라는 NPC가 맡게 되고, 특성 강화는 디시트라는 NPC가 맡게 된다. (100%)~~
- ~~퀘스트를 클리어하여 바뀐 NPC에 따라 상점 관련 대사를 모두 변경한다. (100%)~~
- ~~퀘스트를 클리어하기 위한 아이템 두 가지는 용기의 기억의 사념 조각와 지혜의 기억의 사념 조각으로 각각 적 아이디가 홀수 혹은 짝수일 때 20% 확률로 나온다. (100%)~~

### 35) 44주 강화 작업
**추가 사항**
- ~~A등급 이상의 아이템들의 강화 최대 수치를 변경한다. 단, 이것은 서브 퀘스트를 클리어할 때 오르도록 설정한다. (100%)~~
- ~~Act 5의 최종 보스를 변경한다. (100%)~~
- ~~받은 음원들을 게임에 적용한다. (100%)~~
- ~~서브 퀘스트 대화를 추가 및 영문화 작업을 한다. (100%)~~
- ~~임시로 잠재능력 시스템을 추가한다. (100%)~~

### 36) 45~46주 잔존강념 작업
**추가 사항**
- ~~잔존강념 시스템을 추가한다. 잔존강념 시스템은 여정을 클리어하면 주는 보급 아이템이다. (100%)~~
- ~~잔존강념은 각 챕터 1,2,3,4,5,6에 아이템을 하나 보급받을 수 있는 시스템이다. 1챕터는 게임 시작과 동시에 지급이 되며, 2~6 챕터는 다음 챕터 넘어가기 직전에 수령을 물어보게 만든다. (100%)~~
- ~~잔존강념을 얻는 조건은 8% Wonder 아이템, 22% Rare 아이템, 70% Normal 아이템이다. 조합 아이템은 이것에 포함되지 않으며, 상점 아이템은 상점에서 해금해야 리스트에 추가될 수 있다. (100%)~~
- ~~잔존강념은 같은 아이템을 3개 소비해서 다시 뽑을 수 있다. (100%)~~
- ~~잔존강념은 Act 별로 보상으로 받을 수 있는 아이템이 달라진다. (100%)~~
- ~~Act 1에서는 Normal F 등급 아이템만이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 2에서는 챕터 1에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 3에서는 챕터 1~2에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 4에서는 챕터 1~3에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 5에서는 챕터 1~4에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 6에서는 챕터 1~5에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~Act 7에서는 모든 챕터에서 나오는 아이템들이 잔존강념 보상으로 나온다. (100%)~~
- ~~인트로에서 튜토리얼이 나오도록 선택할 수 있도록 변경한다. 이는 홈 화면 옵션에서도 설정할 수 있다. (100%)~~

**변경 사항**

- ~~포이즌 스파이더, 포이즌 웜, 블랙 울프, 라바 슬라임을 각각 트리벤저, 오래된 골렘, 머쉬룸링, 시드 리저드로 바꾼다. 해당 아이템들의 Unique 아이템들의 설명 역시 이 몬스터들에 맞게 변경한다. (100%)~~
- ~~마녀 대화 리스트를 메인 스토리와 서브 스토리로 구별할 수 있게 변경한다. (100%)~~
- ~~기타 아이템이 모두 얻기를 하면 얻어질 수 있도록 변경한다. (100%)~~
- ~~자동 장착 시스템이 빈 장비 슬롯이 있을 경우, 해당 종류 아이템을 얻으면 바로 장착할 수 있도록 변경한다. (100%)~~
- ~~퀘스트에 초반 보상 및 이후 보상들을 모두 추가한다. (100%)~~
- ~~퀘스트 보상 목록이 다양한 것들이 나오는 것을 대비하도록 변경시킨다. ( 리스트 구조로 ) (100%)~~
- ~~속도 표기에서 스테이지 수를 전체 스테이지로 변경한다. (100%)~~
- ~~상자 모양 아이콘과 랭킹의 속도 버튼의 모습을 바꾼다. (100%)~~
- ~~엔딩, 게임 오버 모습을 반투명한 검은 화면으로 띄우도록 변경한다. (100%)~~
- ~~짙은 부정한 악마의 공격 이펙트가 슬라임으로 되던 버그를 수정한다. (100%)~~
- ~~상점 나갈 때 인트로 화면으로 넘어가던 버그를 수정한다. (100%)~~
- ~~평범한 사념 조각의 가격을 2000으로 롤백한다. (100%)~~
- ~~크리티컬 흔들림으로 뒤에 빈 배경이 보이는 버그를 수정한다. (100%)~~

### 37) 47주 사념 제련소 작업
**추가 사항**
- ~~사념 제련소를 추가한다. 사념 제련소는 2% 확률로 나오는 것으로, 5번의 기회를 주어서 아이템의 내구도를 수리하거나 아이템을 강화할 수 있다. (100%)~~
- ~~사념 제련소의 슬롯에 아이템을 드래그 앤 드랍을 할 수 있도록 만든다. (100%)~~
- ~~사념 제련소의 슬롯에 있는 아이템이 내구도가 없거나 내구도가 모두 찬 상태일 경우, 수리가 불가능하게 버튼을 비활성화시킨다. (100%)~~
- ~~사념 제련소의 슬롯에 있는 아이템이 R등급 이하의 Normal이나 Rare 아이템이 아닐 경우, 강화하기 버튼을 비활성화시킨다. (100%)~~
- ~~사념 제련소의 슬롯에 있는 아이템을 강화하면 2%로 다음 랭크 아이템의 다음 희귀성 아이템이 나오며, 3%로 다음 랭크 아이템의 같은 희귀성 아이템이 나오며, 20%로 다음 희귀성 아이템이 나오며, 25% 확률로 같은 등급이며 같은 희귀성 아이템이 나오며, 39% 확률로 강화에 실패하며, 9%확률로 이전 등급이며 같은 희귀성 아이템이 나온다. 1%로 신규 너클 아이템이, 1%로 아이템이 파괴된다. (100%)~~
- ~~사념 제련소로 내구도를 수리하면, 현재 챕터의 내구도 감소량만큼 내구도가 회복된다. (100%)~~
- ~~사념 제련소에는 오로지 장비 아이템만 올릴 수 있다. (100%)~~
- ~~신규 너클 아이템을 추가한다. 신규 너클 아이템은 S등급 레전드 아이템으로, 공격력 75, 방어력 27, 반격 보정 12%, 크리티컬 데미지 137%, 크리티컬 확률 12%가 증가한다. (100%)~~
- ~~신규 방어구 아이템을 추가한다. 스틸 암즈는 E등급 Wonder 아이템으로, 방어력 9, 크리티컬 데미지 30%가 증가하는 효과가 있다. (100%)~~
- ~~제련에 실패하여 아이템이 파괴되면 달성되는 어이쿠 손이 미끄러졌네 특수 퀘스트를 추가한다. (100%)~~

**변경 사항**

- ~~가방 아이템 종류가 보상의 맨앞에 나오도록 변경한다. (100%)~~
- ~~1~2 챕터에서는 접몽 석상 대신에 사념 제련소가 나오도록 변경한다.( 이때, 기회는 2번으로 만든다 ) (100%)~~
- ~~신규 몬스터들의 오라를 고친다. (100%)~~
- ~~1,2 챕터에서는 사념 제련소가, 그 이후에는 접몽 석상이 나오도록 변경한다. (100%)~~
- ~~눈에는 눈 이에는 이 보상을 40,000으로 늘린다. (100%)~~
- ~~기록소 필터의 설정 폰트가 이전 것이던 버그를 수정한다. (100%)~~
- ~~게임 오버 화면이 더 짙게 만든다. (100%)~~
- ~~인게임 튜토리얼이 옵션에 영향을 받지 않던 버그를 수정한다. (100%)~~
- ~~인게임에서 대화를 스킵 해도 튜토리얼 나오도록 변경한다. (100%)~~
...
드디어 히든 보스가 추가되었다.
다음 주부터는 서버 구축 준비 및 스토리 추가 패치가 예정되어 있다.

### [5주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week57.mp4" type="video/mp4" />
</video>

보스 상호작용 대사가 추가되었다.
또한, B등급 아이템 강화가 추가되었다.
5강 이상은 조건을 만족해야 할 수 있게 만들 예정이라 아직은 최대 4번까지만 가능하다.
서버 구축 준비는 진행중이다.

### [6주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week58.mp4" type="video/mp4" />
</video>

튜토리얼 독백 장면이 추가되었고, 사전이 개편되었다.
게다가 사전을 통해 몽환석을 아주 작게나마 수급할 수 있게 되었다.

다음 패치에서는 마녀와의 상호작용을 추가할 예정이며, 사전에 수거할 몽환석이 있으면 목차 옆에 표시하게 할 예정이다.

### [7주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week59.mp4" type="video/mp4" />
</video>

드디어 스토리가 본격적으로 추가되기 시작했다.
원래부터 있기는 했지만 이제는 스토리가 추가될 차례가 되어서 정말로 기쁘다.
다음 패치에는 드디어 다음 난이도가 추가될 예정이다.
...
이번에는 편의성 패치를 하였다.
이제 다음 주부터 드디어 두 번째 난이도 패치가 이루어질 예정이다.

### [9~10주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week61-1.mp4" type="video/mp4" />
</video>

<video width="100%" height="100%" controls="controls">
  <source src="./video/week61-2.mp4" type="video/mp4" />
</video>

드디어 Act 2가 추가되었다.
이후에 나올 Act들을 위해서 일단 Act 1의 난이도를 낮추긴 했지만...
나쁘지 않을 수도.

### [11주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week63.mp4" type="video/mp4" />
</video>
...
<video width="100%" height="100%" controls="controls">
  <source src="./video/week64-2.mp4" type="video/mp4" />
</video>
...
<video width="100%" height="100%" controls="controls">
  <source src="./video/week65-2.mp4" type="video/mp4" />
</video>

이번 주부터는 개선 작업을 시행할 예정이다.
튜토리얼과 다른 시스템이 개선이 될 예정이다.
...
이번 작업은 도저히 20초 내로 보일 수가 없어서 링크로 대체한다.
팀원의 도움으로 게임이 많이 발전한 것 같다.

### [15주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week69.mp4" type="video/mp4" />
</video>

새로운 조우 요소인 특수 상자를 추가하였다.
이 특수 상자는 추후에 내용물 중에 Unique 아이템을 추가할 예정이다.

### [16주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week70.mp4" type="video/mp4" />
</video>

이번에는 아이템 확대 창과 새로운 조합 아이템을 추가하였다.
이 다음에는 버그를 고칠 예정이다.

### [17주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week71.mp4" type="video/mp4" />
</video>

수많은 버그를 모두 고쳤다.
이제 다시 본 작업에 돌입할 예정이다.

### [18주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week72.mp4" type="video/mp4" />
</video>

튜토리얼 작업이 모두 마무리되었다.
이제 다음에는 기록소 콘텐츠 추가, 가방 시스템 개선 패치를 진행할 예정이다.

### [19주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week73-2.mp4" type="video/mp4" />
</video>

기록소에 인물 사전이 추가되었고, 게임이 유료로 변경되었음에 따라 재화를 전보다 더 많이 주도록 변경하였다.
다음에는 가방 시스템 개선 작업이 있을 예정이다.

### [20주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week74.mp4" type="video/mp4" />
</video>

이제 가방 시스템이 더 편해지도록 바뀌었고, 튜토리얼 보상이 생겼다.
또한, 탐색과 제약, 그리고 마강이 해금용 특성으로 변했다.

### [21주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week75.mp4" type="video/mp4" />
</video>

이제 지식 사전이 추가되어서 세계관을 더 자세히 이해할 수 있게 되었다.
다음 주부터는 영문화 작업을 진행할 예정이다.

### [22~23주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week76.mp4" type="video/mp4" />
</video>

드디어 영문화가 완료되었다...!
이제부터 스프라이트를 변경할 예정이다...
그리고 드디어 Act 3를..!!!

### [24~25주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week78.mp4" type="video/mp4" />
</video>

스프라이트도 변경되었다.
근데 잠시 무언가를 좀 추가하고 Act 3를 추가할 예정이다.

### [26주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week80.mp4" type="video/mp4" />
</video>

드디어 보스 기억이 추가되었다.
이후에는 퀘스트 시스템을 추가할 예정이다.

### [27주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week81.mp4" type="video/mp4" />
</video>

UI를 조금 리모델링(?)하였다.
이후에는 진짜로 퀘스트 시스템을 추가할 예정이다.

### [28주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week82.mp4" type="video/mp4" />
</video>

반복 퀘스트와 목표 퀘스트가 추가되었다.
다음에는 일일 퀘스트를 추가할 예정이다.

### [29주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week83.mp4" type="video/mp4" />
</video>

드디어 모든 퀘스트가 추가되었으며, 아이템을 모두 바꾸었다.
이후는 이제 시간이 걸리는 패치만 있어서 조금씩 할 예정이다.

### [30주차 구현과정]

<video width="100%" height="100%" controls="controls">
  <source src="./video/week84.mp4" type="video/mp4" />
</video>

전시회의 의견을 다음 주까지 모두 반영할 예정이다.
Act 3는 또 미뤄졌다.
...
<video width="100%" height="100%" controls="controls">
  <source src="./video/week87.mp4" type="video/mp4" />
</video>

Act 3 기획은 끝났고, 일단 개선할 것들을 모두 개선했다.
이제 Act 3를 계속 제작할 것이고, 패치는 작은 것들만 할 예정이다.
