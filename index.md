...
- 기록 버튼을 누르면 다른 팝업창과 같은 크기를 가진 기록 팝업창이 나온다.
...
- 게임 오버 또는 포기를 하면 나오는 팝업창 중앙에 플레이어의 이름, 플레이어의 점수, 플레이어의 특성, 플레이어가 최종적으로 도달한 챕터, 플레이어가 얻었던 가장 높은 랭크의 아이템이 출력되며, 아래에는 플레이어가 랭크별로 얻은 아이템의 수와 챕터별로 처치했던 몬스터의 수가 표시된다. ( 보스 몬스터는 따로 표시한다. )
- 게임 승리를 하면 이름과 점수만 나오게 표시한다.
