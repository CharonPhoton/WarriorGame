...
### 27) 31주 전시회 의견 반영 2차
**추가 사항**
- 조합할 때 재료를 모두 자동으로 넣는 버튼을 추가한다.
- 자동 넣기 버튼을 누르면 내구도가 가장 많이 닳은 아이템이 인벤토리에서 우선적으로 재료로 들어가게 된다.
- 보상을 수령하는 창에서 가방, 마법, 기타 아이템을 제외한 아이템들을 바로 사용할 수 있는 버튼을 추가한다.
- 보상에서 장비 아이템을 바로 장착할 경우, 해당 종류의 장비가 비어 있는 상태면 바로 장착된다.
- 보상에서 장비 아이템을 바로 장착할 경우, 해당 종류의 장비가 장착된 상태일 경우면 인벤토리가 하나 이상 빈 상태에서 장비를 교체하게 바꾼다.

**변경 사항**

- 일일 퀘스트와 반복 퀘스트 보상이 일부 증가한다. ( 각각 40 -> 50, 20% 증가 )
- 기록소에서 보상 수령 버튼이 해당 창에서 받을 수 있는 보상이 있을 경우 출력되게 변경된다.
- 가방을 닫을 때 드래그된 아이템이 있으면 초기화되게 변경한다.
- 인게임의 옵션 창과 아이템을 얻을 수 없는 창들은 ESC로 끌 수 있도록 변경된다.
- 모바일에서 단축키가 텍스트로 나오는 버그를 수정한다.
- 첫발이 이제 슬로우, 베리 슬로우, 트라이얼에서는 각각 4,5,6 스테이지까지 상자 조우 확률을 올리도록 변경한다.
