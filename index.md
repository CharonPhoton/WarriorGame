...
### 29) 33주 슴슴히 개선
**추가 사항**
- 새로운 조합 아이템 블랙 블러드 아머를 추가한다. 블랙 블러드 아머는 S등급 Wonder 방어구 아이템으로, 방어력을 70 증가해준다. ( 조합법은 철 갑옷 + 붉은 보석 + 강철 갑옷 + 블랙 부츠 )
- 새로운 조합 아이템 히어로즈 피스톨을 추가한다. 히어로즈 피스톨은 SR등급 Wonder 총 무기 아이템으로, 공격력 92, 크리티컬 데미지 100% 증가해준다. ( 조합법은 리볼버 + 샤프 슈터 + 수르쿨루스 + 영웅의 동전 )

**변경 사항**

- 인벤토리가 꽉 차거나 가방이 최대치에 도달한다는 팝업창을 삭제하고 안내를 몇 초 뒤에 사라지는 텍스트로 출력되도록 변경한다.
- SSR와 SR 등급의 색깔을 변경한다.
- 첫발 특성 설명을 변경한다.
- 상기 상자의 아이템들이 이미지와 구입 버튼 사이에 이름까지 출력되도록 변경한다.
- 챕터 6의 배경을 변경한다.
- Act 1의 마왕의 이미지를 변경한다.
- 마법 아이템이 Z 버튼을 눌러서 작동하지 않던 버그를 수정한다.
- 랭킹의 양피지 UI를 오래된 양피지 이미지로 변경한다.
- 배경의 뒷배경이 몬스터보다 앞에 나오도록 변경한다.
- 보스 때 배경이 붉게 변하는 것이 약하게 변하는 것으로 변경한다.

### 30) 34~35주 Act 3 제작
**추가 사항**
- Act 3의 몬스터들의 밸런스를 조절한다.
- Act 3에 나오는 대화들을 추가한다.
- Act 3에 나오는 새로운 전투 시스템인 게이지와 새로운 조작인 방어하기를 추가한다.
- 방어하기를 하면 무조건 적에게 공격을 받지만, 받는 피해가 85%까지 감소하며, 적의 게이지가 두 칸 깎인다.
- 공격하기 버튼을 누르면 적의 게이지가 한 칸 쌓인다.
- 게이지가 최대치에 도달하면, 게이지를 리셋하고 적이 필살기를 사용한다.
- Act 2이후의 대화를 세 가지 추가한다.
- 아이템 사용 시에 이펙트가 나오도록 변경한다.

**변경 사항**

- 모든 특성이 이제 최대 1까지 강화가 가능하도록 변경한다.
- 반사 버프가 받는 데미지가 없어도 적용되도록 변경한다.
- 아이템 장착에 이펙트를 추가하며, 기존의 파괴 이펙트를 새로운 이펙트로 대체한다.
- 진행하기 버튼 디자인을 변경한다.
- 강몽석 가격을 160으로 낮춘다.
- 강몽석 획득 점수 기준을 35,000에서 30,000으로 낮춘다.

### 31) 36~37주 Act 4 제작
**추가 사항**
- Act 4의 몬스터들의 밸런스를 조절한다.
- Act 4에 나오는 대화들을 추가한다.

**변경 사항**

- 아이템 강화의 강몽석 필요 가격을 모두 25% 낮춘다.
- 강몽석 가격을 150으로 낮춘다.
- 강몽석 획득 점수 기준을 30,000에서 20,000으로 낮춘다.
- 의문의 상자에서 나오는 강몽석의 갯수를 증가시킨다.
