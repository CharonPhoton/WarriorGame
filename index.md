...
**추가 사항**
- 인물 사전을 추가한다.
- 인물 사전의 인물들은 조건에 따라서 열리게 만들며, 잠겨진 인물들은 아이템이나 몬스터처럼 모두 비공개로 만든다.
- 잠겨진 인물은 이미지가 검게 출력되며, 비공개된 텍스트는 모두 ???로 만든다.
- 인물의 스토리 설명은 분기마다 구분선을 둔다.
- 인물의 내용에는 이름, 이미지, 대표적인 말, 능력, 성격, 좋아하는 것, 싫어하는 것, 대표 설명, 스토리 설명이 있다.
- 모든 필터창, 팝업창, 설정창을 바깥을 누르면 Cancel 버튼 시스템이 작동되도록 만든다.
- 접몽 석상에서 조합을 진행하지 않은 상태일 경우, 돌아가는 버튼이 켜지도록 만든다. 또한, 돌아가기 버튼을 누르면 다시 상자를 선택할 수 있는 곳으로 돌아오도록 만든다.

**변경 사항**

- 몬스터들의 강몽석 드랍 확률을 2%로 증가시킨다.
- 어둠 속에 존재하는 자의 강몽석 드랍 확률을 100%로 변경한다.
- 의문의 상자의 몽환석과 강몽석의 지급량을 1.5 ~ 2배만큼 증가시킨다.
- 점수 비례 몽환석 지급량을 점수 / 90에서 점수 / 55로 낮춘다.
- 점수 비례 강몽석 지급량을 점수 / 55,000에서 점수 / 35,000로 낮춘다.
- 사전의 아이콘이 눌러진 상태면 밝은 상태로 변경되도록 수정한다.
- 대사에 생각과 대화 표현 구분한다.
- 너클류 무기 아이템의 이름 변경한다.
- 이름 입력칸을 삭제한다.
- 접몽 석상 선택 시 지급되던 점수를 더 이상 주지 않게 만든다.
- 보스 아이템 조우 대화가 Act 1에서만 보여지도록 변경한다.
- 사전의 맨 오른쪽에 있는 아이템의 이름이 잘리는 버그 수정한다.
