...
### 34) 43주 개선 작업
**추가 사항**
- Act 3 중간에 나오는 퀘스트를 추가한다.
- 퀘스트를 진행하는 도중에는 홈 화면에 해당 퀘스트를 위한 아이템 개수가 표시된다.
- 퀘스트를 클리어하기 위한 아이템이 30개 이상 모였을 경우, 상점에서 대화를 진행하여 강화 한계를 증가시킬 수 있고 캐릭터 기본 능력치를 올릴 수 있도록 만든다.
- 퀘스트를 클리어한 이후에는 상점의 아이템 강화는 피르마라는 NPC가 맡게 되고, 특성 강화는 디시트라는 NPC가 맡게 된다.
- 퀘스트를 클리어하여 바뀐 NPC에 따라 상점 관련 대사를 모두 변경한다.
- 퀘스트를 클리어하기 위한 아이템 두 가지는 용기의 기억의 사념 조각와 지혜의 기억의 사념 조각으로 각각 적 아이디가 홀수 혹은 짝수일 때 15% 확률로 나온다.
