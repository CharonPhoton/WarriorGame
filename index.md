...
**추가 사항**
- 모든 몬스터 스프라이트, 챕터 배경 스프라이트 ( 어둠 속에 존재하는 자 전투 배경 포함 )을 변경한다.
- Act 2에서 등장하는 모든 몬스터들의 밝기를 조금 줄이고, 희미한 검은 연기가 피어오르는 연출을 추가한다.
- 몬스터의 애니메이션이 변경됨에 따라서 몬스터 사망 연출을 통제하는 코드를 수정한다.
- 

**변경 사항**

- Act에 소제목을 추가한다.
- 첫 번째 로딩에서는 시놉시스가 나오도록 변경한다.
- 마녀 대화의 대화 제목이 안 나오던 버그를 수정한다.
- 영문에서 마녀 대화가 읽음 처리로 되지 않던 버그를 수정한다.
- 조합의 빼기 버튼이 영문화가 안 되던 버그를 수정한다.
- 빼기 버튼을 사용하면 조합 닫기 버튼이 고장나는 버그를 수정한다.
- 아이템, 특성 강화 시의 표기 버그를 수정한다.
- 가방 받기 버그를 수정한다.
- 기록소 지식 쪽의 용사가 있는 곳이 영어명 때에는 이름 칸을 뛰어넘던 버그를 수정한다.
- 특성 튜토리얼이 출력되지 않던 버그를 수정한다.
- 영문에서 용사는 Hero에서 Warrior로, 전사는 Warrior에서 Striker로 변경한다. 
