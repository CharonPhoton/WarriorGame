...
### 21) 26주 보스 기억 아이템 추가
**추가 사항**
- 보스 몬스터에게서 기억 아이템을 드랍하게 한다. ( Act별로 다른 아이템이 나오도록 만든다. )
- 보스 몬스터의 기억 아이템을 얻으면, 해당 아이템은 인벤토리에 들어가지 않고 삭제된다. 해당 아이템을 얻으면 기록소에서 해당 아이템에 맞는 기억이 해금된다.
- 해금된 보스 기억은 소설의 형태로 볼 수가 있다.
- 보스 기억을 해금하면 보상으로 몽환석 200을 받을 수가 있다.

**변경 사항**

- 전투 확률 바를 복구시킨다.
- 홈 화면, 상점 화면, 치유의 숲 배경, 이상한 공간의 배경을 모두 바꾼다.
- 가방 버그를 수정한다.
- Act 2 어둠 속에 존재하는 자의 전투 패배 이후 오라가 사라지지 않던 버그를 수정한다.
