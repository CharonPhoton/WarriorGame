...
**추가 사항**
- 일일 퀘스트를 추가한다. 일일 퀘스트는 다음 날 오전 9시 이후면 갱신되도록 만들며, 인터넷이 연결되어 있을 때 갱신되도록 설정한다.
- 특수 퀘스트를 추가한다. 특수 퀘스트는 정말 달성하기 어려운 임무인 퀘스트만이 출력된다.
- 일일 퀘스트는 갱신이 되면 10개의 퀘스트 중 랜덤으로 네 개의 퀘스트를 뽑아 갱신하며, 일일 퀘스트를 클리어하는 퀘스트는 고정적으로 추가한다.
- 일일 퀘스트의 목록에는 일일 퀘스트 클리어, 몬스터 처치, 보스 처치, 아이템 얻기, 무기나 방어구 아이템 얻기, 장신구나 부적 아이템 얻기, 상자 발견하기, 체력 아이템 사용하기, 가방 아이템 얻기, 마법 아이템 얻기, 장비 아이템 파괴되기가 있다.
- 특수 퀘스트의 목록에는 해당 보스 몬스터의 아이템을 착용한 상태로 그 보스를 쓰러뜨리기가 각 보스마다 있다.
- 조합 아이템 중에서 재료가 이전 챕터까지만 나오는 재료가 있으면 아이템 정보나 조합 목록에서 나오는 재료를 붉게 표시하며, 조합 아이템 목록 상단에 재료가 더 이상 안 나오는 아이템이라고 표기한다.

**변경 사항**

- 모든 아이콘을 새로 만들어진 도트 아이콘으로 변경한다.
- 상태이상이나 버프의 잔여 턴을 중앙에서 우측 하단에 표기하도록 설정한다.
- 아이템 조합법에서 보이는 관심 버튼을 누르면 배경이 보이도록 변경한다.
- 반복 퀘스트의 보상을 25% 증가한다.
- 일부 UI의 디자인을 변경한다.
- 일부 UI의 폰트 색을 변경한다.
