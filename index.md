 ...
 **추가 사항**
 - 몬스터 등장 연출을 추가한다.
 - 일반 몬스터가 등장할 경우, 뒤에서 앞으로 오는 느낌이 들게 만든다. 또한, 흙먼지를 일으키는 효과를 추가해서 빠르게 가까워진다는 느낌을 받게 만든다.
 - 보스 몬스터가 등장할 경우, 검은 상태에서 서서히 모습이 보이도록 만들게 한다. 이때, 뒤에 보라색 마법진이 돌아가는 연출을 추가한다.
 - 마왕이 등장할 경우, 검고 투명한 상태에서 서서히 모습이 보이도록 만들게 한다. 이때, 보스 몬스터 연출보다 더 큰 진한 보라색 마법진이 뒤에서 돌아가는 연출을 추가한다.
 - 히든 보스가 등장할 경우, 빛나는 두 원이 교차하며 보라색 연기가 나오는 연출이 나오며, 히든 보스가 투명한 상태에서 서서히 모습이 보이도록 만든다.
 - 너클 타입 무기를 추가한다. 너클 타입 무기는 체력과 마력을 제외한 모든 능력치를 올리는 대표 효과를 가지고 있으며, 희귀성이 모두 Unique 이상이다.
 - 너클 타입 무기의 공격 이펙트는 무기가 없을 때의 이펙트와 동일하다.
 - 히든 보스로 어둠 속에 존재하는 자를 추가한다. 어둠 속에 존재하는 자를 조우하면 배경이 붉은 하늘로 변한다.
 - 히든 보스의 몬스터 아이콘은 후드를 쓴 자의 모습을 한 검은 아이콘이다.
 - 어둠 속에 존재하는 자는 체력이 5,000이며, 공격력이 50이며, 반격 확률이 70%이며, 치명타 확률이 38%다.
 - 어둠 속에 존재하는 자에게 패배 혹은 완전히 패배( 재몽석까지 사용했을 경우 )할 경우, 체력을 100%로 채우고 전투를 종료한다.
 - 어둠 속에 존재하는 자를 쓰러뜨리면 100% 확률로 ????를 얻게 된다.
 - ????는 SR등급 Legend 아이템으로, 공격력 97, 방어력 37, 반격 보정 17%, 크리티컬 177%, 크리티컬 확률 17% 상승시키는 너클 타입 무기 아이템이다.

**변경 사항**

- 기억 획득 여부, 클리어 여부를 데이터로 저장할 수 있게 변경한다.
- 최종보스인 마왕이 사용하는 아이콘을 추가하여 변경한다. 아이콘은 중장갑 투구를 착용한 보라색 존재의 아이콘이다.
- 마왕의 이름을 보라색으로 변경한다.
