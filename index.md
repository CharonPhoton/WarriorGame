...
- 검, 방패, 단검, 지팡이 타입의 무기를 추가한다.
- 검 타입은 기본적으로 공격력만을 제공하는 무기 아이템이다.
- 방패 타입은 기본적으로 부가 효과로 방어력을 제공하는 무기 아이템이다.
- 단검 타입은 기본적으로 부가 효과로 반격 보정 확률 증가를 제공하는 무기 아이템이다.
- 검 타입의 무기로 공격하면 무언가를 베는 듯한 소리가 출력된다.
- 검 타입의 무기로 공격하면 적을 베는 이펙트가 출력되며, 크리티컬이 발생될 경우 적을 베면서 무언가가 뿜어져 나오는 듯한 이펙트가 출력된다. 이때, 이펙트의 각도는 랜덤으로 출력된다.
- 방패 타입의 무기로 공격하면 무언가를 둔탁하게 내리치는 듯한 소리가 출력되며, 크리티컬이 발생될 경우 무언가를 내리치면서 철이 부딪치는 듯한 소리가 출력된다.
- 방패 타입의 무기로 공격하면 적을 둔탁하게 내리치는 이펙트가 출력되며, 크리티컬이 발생될 경우 충격파와 함께 적을 내리치는 듯한 이펙트가 출력된다.
- 단검 타입의 무기로 공격하면 무언가를 베는 듯한 소리가 출력된다.
- 단검 타입의 무기로 공격하면 빠르게 적을 베는 이펙트가 출력되며, 크리티컬이 발생될 경우 빠르게 베며 충격파가 일어나는 듯한 이펙트가 출력된다. 이때, 이펙트의 각도는 랜덤으로 출력된다.
- 지팡이 타입의 무기로 공격하면 마법을 사용하는 듯한 신비한 소리가 출력된다.
- 지팡이 타입의 무기로 공격하면 신비한 무언가로 적을 타격하는 듯한 이펙트가 출력되며, 크리티컬이 발생될 경우 마법진이 발생과 함께 충격파로 적을 타격하는 듯한 이펙트가 출력된다.
- 기존에 있던 플레이어의 공격, 치명타 공격 이펙트는 무기 없이 공격했을 때 출력된다.
- 무기 없이 공격해서 크리티컬이 발생될 경우, 강하게 내리치는 소리가 출력된다.
- 플레이어 잔류 효과는 플레이어의 점수 위에 오른쪽부터 출력되게 한다.( 단, 전투 중일 경우에는 성공 확률을 나타내는 UI 위에 출력하게 한다. )
- 출혈, 기절 상태이상을 추가한다.
- 출혈에 걸리게 되면 개체는 전투 결과 이후에 기존 체력의 10%만큼의 데미지를 받는다. ( 단, 보스 몬스터들은 체력의 5%만큼의 데미지를 받는다. )
- 기절에 걸린 개체는 반격 실패 확률이 100%가 되어 다음 공격에 무조건 공격을 받는다. ( 몬스터가 이 상태이상에 결렸을 경우 반격 확률을 0으로 만들며, 플레이어가 이 상태이상에 결렸을 경우 몬스터의 반격 확률을 100%로 한다. )
- 기절에 걸린 적에게는 위에 노란 원이 빙빙 돌고 있고, 별이 뿜어져 나오는 연출이 추가된다.
- 출혈에 걸린 적은 피를 뿜어내는 듯한 붉은 연기 연출이 추가된다.
- 투지, 수호, 반사, 회복 잔류 효과를 추가한다.
- 투지 잔류 효과를 적용받고 있을 경우, 공격력이 40% 증가한다.
- 수호 잔류 효과를 적용받고 있을 경우, 방어력이 30% 증가한다.
- 반사 잔류 효과를 적용받고 있을 경우, 데미지를 받으면 그 데미지의 75%만큼의 피해를 적에게 준다.
- 혼흡 잔류 효과를 적용받고 있을 경우, 공격에 성공할 때마다 8%만큼의 체력을 회복한다.
- 새로운 마법 아이템 6개를 추가한다.
- 스트렝스 마법 아이템을 추가한다. 스트렝스는 마력 3가 소모되며, 3턴 동안 투지 효과를 플레이어에게 부여하는 효과를 가지고 있다. 등급은 D다.
- 프로텍트 마법 아이템을 추가한다. 프로텍트는 마력 3가 소모되며, 3턴 동안 수호 효과를 플레이어에게 부여하는 효과를 가지고 있다. 등급은 D다.
- 리플렉션 마법 아이템을 추가한다. 리플렉션은 마력 3가 소모되며, 3턴 동안 반사 효과를 플레이어에게 부여하는 효과를 가지고 있다. 등급은 D다.
- 파이트 리커버리 마법 아이템을 추가한다. 파이트 리커버리는 마력 3가 소모되며, 3턴 동안 혼흡 효과를 플레이어에게 부여하는 효과를 가지고 있다. 등급은 D다.
- 쇼크 브레이크 마법 아이템을 추가한다. 쇼크 브레이크는 마력 4가 소모되며, 적에게 기절 상태이상을 부여하는 효과를 가지고 있다. 등급은 C다.
- 블러드 해프닝 마법 아이템을 추가한다. 블러드 해프닝은 마력 4가 소모되며, 적에게 3턴 동안 출혈 상태이상을 부여하는 효과를 가지고 있다. 등급은 C다.
- 스트렝스, 프로텍트, 리플렉션, 파이트 리커버리는 전투 중이 아니라도 사용할 수 있다.
- 투지, 수호, 반사, 혼흡 효과는 전투가 끝나더라도 카운트가 남아 있으면 잔류하게 된다.
- 투지 효과를 받을 경우, 투지 효과를 받는다는 텍스트를 주황색 글씨로 출력한다.
- 수호 효과를 받을 경우, 수호 효과를 받는다는 텍스트를 노란색 글씨로 출력한다.
- 반사 효과를 받을 경우, 반사 효과를 받는다는 텍스트를 하늘색 글씨로 출력한다.
- 혼흡 효과를 받을 경우, 호흡 효과를 받는다는 텍스트를 붉은색 글씨로 출력한다.
- 플레이어가 적의 공격을 반사했을 경우 흰색 글씨로 반사되어 적이 데미지를 받았다는 텍스트를 출력한다.
- 자신이 좋은 잔류 효과를 받았을 경우, 왼쪽화면이 초록색으로 깜빡거리게 만든다.
- 출혈 상태이상에 걸렸을 경우, 출혈 상태이상에 걸렸다는 텍스트를 붉은색 글씨로 출력한다.
- 기절 상태이상에 걸렸을 경우, 기절 상태이상에 걸렸다는 텍스트를 주황색 글씨로 출력한다.
- 새로운 무기 아이템을 한 개 추가한다.
- 고목 방패는 D등급의 Wonder 아이템으로, 공격력 9, 방어력 3을 제공하는 방패 타입 무기다.
- 슬라임, 숲 전갈, 버섯 인간 스포아, 라바 슬라임, 파이어 케로베로스, 사무친 원혼, 마스터 케로베로스가 스트렝스를 드랍하게 된다. ( 확률은 15% )
- 들판 거미, 티스 웜, 흉포한 쥐, 서늘한 원혼, 본웜, 원한의 유령, 부정한 악마가 프로텍트를 드랍하게 된다. ( 확률은 15% )
- 그린 슬라임, 포이즌 스파이더, 블루 스콜피온, 유령, 고블린 바드, 디스커스팅 본웜, 매드 본웜이 리플렉션을 드랍하게 된다. ( 확률은 15% )
- 거대 들쥐, 그린 울프, 블랙 울프, 레드 스파이더, 라스 웜, 스켈레톤, 크리스털 스콜피온이 파이트 리커버리를 드랍하게 된다. ( 확률은 15% )
- 킹스 아이, 썩은 쥐, 세 머리의 울프, 불안정한 원혼, 해골전사 켈트, 리더 울프가 쇼크 브레이크를 드랍하게 된다. ( 확률은 15% )
- 블루 울프, 블랙 슬라임, 그로겔, 포이즌 그로겔, 프로즌 랫, 크리스털 스파이더, 다크니스 그로겔이 블러드 해프닝을 드랍하게 된다. ( 확률은 15% )
- 포이즌 웜이 금괴를 드랍하게 변경한다. ( 확률은 30% )

**변경 사항**

- 기존 주괴 아이템을 검 아이템으로 바꾼다.
- 기존 검 아이템을 대검 아이템으로 바꾼다.
- 기존 도끼 아이템을 레이피어 아이템으로 바꾼다.
- 기존 대검 아이템을 월도 아이템으로 바꾼다.
- 기존 푸른 마법책 아이템을 활 아이템으로 바꾼다.
- 기존 레이피어 아이템을 슬래셔라는 단검 타입의 아이템으로 바꾼다.
- 기존 활 아이템을 롱보우라는 활 타입의 아이템으로 바꾼다.
- 기존 월도 아이템을 바바리안 스워드 아이템으로 바꾼다.
- 기존 바바리안 스워드 아이템을 히트 라이플이라는 총 타입의 아이템으로 바꾼다.
- 기존 고대서 아이템을 은 검 아이템으로 바꾼다.
- 기존 라이트 해머 아이템을 라이트 라이플이라는 총 타입의 아이템으로 바꾼다.
- 기존 신성한 도끼 아이템을 에이션트 소드 아이템으로 바꾼다.
- 기존 커트리스 보텀 아이템을 아머드 보우라는 활 타입의 아이템으로 바꾼다.
- 기존 붉은 마법책 아이템을 포에나 아이템으로 바꾼다.
- 기존 베네디캇 아이템을 실렌티움이라는 총 타입의 아이템으로 바꾼다.
- 기존 홀리 액스 아이템을 데빌 브레이커 아이템으로 바꾼다.
- 기존 스페스의 눈 아이템을 스페스의 검이라는 검 타입의 아이템으로 바꾼다.
- 기존 거미의 독니 아이템을 독 바른 검이라는 검 타입의 아이템으로 바꾼다.
- 기존 포이즌 글라디우스 아이템을 포이즌 불렛 라이플이라는 총 타입의 아이템으로 바꾼다.
- 기존 흉포한 도끼 아이템을 브루탈 라이플이라는 총 타입의 아이템으로 바꾼다.
- 기존 골검 아이템을 골궁이라는 활 타입의 아이템으로 바꾼다.
- 기존 스켈레톤 블레이드를 스켈레톤 라이플이라는 총 아이템으로 바꾼다.
- 기존 마스터 케로베로스의 단검을 마스터 케로베로스의 인내라는 방패 아이템으로 바꾼다.
- 기존 불의 격통 아이템이 검의 형태의 아이템으로 변경하며, 검 타입의 아이템으로 변경한다.
- 기존 부정한 악마의 마법의 이름이 부정한 악마의 해골로 변경한다.
- 기존 상태 이상 및 능력치 설명을 하던 UI를 위에는 이름, 아래는 효과가 출력되게 변경한다. ( 그 사이에는 테두리로 구분을 해둔다. )
- 특성을 선택할 때, 결정할지 말지 UI를 띄운다. 결정을 누르면 게임이 시작된다. 이때, 기존에 있던 이름을 입력하는 UI는 결정 선택 UI 칸에 띄운다.
- 화상, 동결, 감전 효과가 4턴 동안 지속되도록 변경
- 포이즌 스파이더가 매직 스워드 대신에 고목 방패를 드랍하게 변경( 확률은 20% )
- 5챕터에서 나오는 장비 아이템들의 출현 확률이 2% 증가
- 썬더 스트라이크의 공격력을 70에서 95로 상향한다.
