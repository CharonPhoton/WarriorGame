...
- ~~좌측 화면 하단 체력 위에 슬라이더를 두 개 겹친 전투 성공 확률을 화면에 표시한다. (100%)~~
...
- 전투 시에는 전투 슬라이더, 확률 위에 공격하기 버튼이 생긴다.
- 전투 시에는 좌측 화면 상단에 적의 체력바와 그 체력바에 그 적의 이름을 표시한다.
- 전투 시에는 적 체력바 아래에 적의 공격력을 표시한다.
