...
### 14) 22주 밸런스 작업
- 모든 몬스터의 즉사 확률을 치명타 확률으로 바꾼다. 그리고 모든 몬스터의 치명타 확률을 모두 5% 올린다. ( 단, 블루 울프의 치명타는 올리지 않는다. )
- 치명타 확률은 플레이어에게 해당 몬스터의 공격력의 두 배의 수치만큼의 데미지를 준다. 이때, 플레이어에게 방어력이 있고 데미지 계산을 할 경우, 치명타로 올라 간 데미지로 계산한다.
- 화면에 표시된 즉사 반격 텍스트를 치명타 반격으로 바꾼다.
- 치명타 발생 시 띄어지는 텍스트를 "OO(이)가 죽음의 일격을 날렸습니다!"에서 "OO의 치명적인 일격! OO 데미지를 받았습니다!"로 바꾼다.
...
- ( 오류 수정 ) 스테이터스 영구 지속 효과가 죽은 이후에도 계속 적용되던 버그를 수정한다.
