...
- 플레이어가 만약에 인터넷에 연결된 상태가 아니라면 로그인 창을 열 수 있는 버튼을 비활성화시킨다.
- 플레이어가 이미 저장을 한 상태라면 전역 변수로 변수를 false로 만들고, 서버 데이터 저장과 로드 버튼을 비활성화 시킨다.
- 이후에 데이터에 변화가 있을 경우 이 변수를 true로 바꿔서 서버 데이터 저장과 로드 버튼을 활성화 시킨다.
- 플레이어의 레코드 데이터에 변화가 있을 때, 그리고 처음 회원가입을 했을 때 노말, 스피드, 베리 스피드에서 가장 높은 랭킹의 데이터를 서버로 전송한다.
- 홈 화면에 플레이어가 서버 랭킹 20위까지의 데이터를 볼 수 있는 창을 열 수 있는 버튼을 추가한다.
